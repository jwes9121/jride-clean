...
Write-Host "== PATCH JRIDE: Dispatch STATUS allow driver secret (V1.2 / PS5-safe) =="
...
$bak = Join-Path $bakDir ("route.ts.bak.DISPATCH_STATUS_ALLOW_DRIVER_SECRET_V1_2." + $ts)
...
if ($content -notmatch "function\s+allowDispatchStatus\s*\(") {
  ...
  $helper = @'
/* JRIDE_DISPATCH_STATUS_AUTH_HELPER_V1_2 */
...
/* JRIDE_DISPATCH_STATUS_AUTH_HELPER_V1_2_END */
...
# 2) Insert auth guard at start of POST handler
...
if ($content -notmatch "JRIDE_DISPATCH_STATUS_AUTH_GUARD_V1_2") {
  $insertAt = $mPost.Index + $mPost.Length
  $guard = @'

  /* JRIDE_DISPATCH_STATUS_AUTH_GUARD_V1_2 */
  if (!allowDispatchStatus(req)) {
    return NextResponse.json({ ok: false, error: "UNAUTHORIZED" }, { status: 401 });
  }
  /* JRIDE_DISPATCH_STATUS_AUTH_GUARD_V1_2_END */
...
# 3) Insert ping handler after req.json()
if ($content -notmatch "JRIDE_DISPATCH_STATUS_PING_V1_2") {
  ...
  /* JRIDE_DISPATCH_STATUS_PING_V1_2 */
  if (body && body.ping === 1) {
    return NextResponse.json({ ok: true, pong: true }, { status: 200 });
  }
  /* JRIDE_DISPATCH_STATUS_PING_V1_2_END */
...
Write-Host "2) Optional dev: JRIDE_ALLOW_UNAUTH_DISPATCH_STATUS=1"
