# PATCH-JRIDE_TAKEOUT_PHASE2B_MENU_CONSUMPTION.ps1
# Phase 2B: Passenger menu consumption (read-only today menu + qty selection)
# Creates: app/api/takeout/menu/route.ts
...
# Create menu API dir
$menuDir = Join-Path $root "app\api\takeout\menu"
if (!(Test-Path $menuDir)) { New-Item -ItemType Directory -Force -Path $menuDir | Out-Null; Ok "Created dir: $menuDir" }
$menuApi = Join-Path $menuDir "route.ts"

# Backups
Copy-Item -Force $takeoutPage "$takeoutPage.bak.$ts"
Ok "Backup: $takeoutPage.bak.$ts"
if (Test-Path $menuApi) {
  Copy-Item -Force $menuApi "$menuApi.bak.$ts"
  Ok "Backup: $menuApi.bak.$ts"
}

# --- Write API: app/api/takeout/menu/route.ts
$api = @'
...
export async function GET(req: NextRequest) {
  ...
  // Be forgiving with column presence.
  ...
  const items = rows.map((r: any) => ({
    ...
    sold_out_today:
      ...
      null,
    last_updated_at: r.last_updated_at ?? r.updated_at ?? r.menu_updated_at ?? null,
  }));
...
[System.IO.File]::WriteAllText($menuApi, $api, $utf8NoBom)
Ok "Wrote: $menuApi"

# --- Write Takeout page: menu consumption UI (keeps Phase 2B.0 address DB integration)
$ui = @'
"use client";
...
Ok "Phase 2B menu consumption patch applied."
Write-Host ""
Write-Host "Next: npm run build, then test /takeout with a real vendor_id that has menu items." -ForegroundColor Cyan
