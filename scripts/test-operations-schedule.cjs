...
test('past assignments cannot be changed after the day ends', () => assert.throws(() => apply(claimed, admin, 'override', { day: '2026-10-01', duty: 'primary', employee: 'coordinator-2', note: 'Late edit' }, new Date('2026-10-02T07:00Z')), /Past schedules/));
test('Admin can correct an ended duty from today with a reason', () => {
  const corrected = apply(claimed, admin, 'override', { day: '2026-10-01', duty: 'primary', employee: 'coordinator-2', note: 'Corrected after shift' }, new Date('2026-10-01T08:00Z'));
  assert.equal(corrected.slots['2026-10-01/primary'].owner, 'coordinator-2');
});
