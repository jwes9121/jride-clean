# PATCH-JRIDE_PHASE3M2_RECONCILE_WALLETS_EXPECTED_PAYOUT_V1.ps1
# Fix reconcile-wallets logic:
# - Only flags missing driver credits when expected_driver_payout > 0
# - Duplicate detection counts ONLY positive "credit-like" tx
# UTF-8 without BOM
...
export async function GET(req: NextRequest) {
  try {
    ...
    const missing_driver_credits = completed
      ...
      .filter((x: any) => x.expected > 0) // ✅ only flag when something is actually expected
      .filter((x: any) => {
        const bid = s(x.b.id);
