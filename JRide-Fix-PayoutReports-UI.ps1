# JRide-Fix-PayoutReports-UI.ps1
# Fixes: app/admin/payouts/drivers/reports/page.tsx
# - Removes syntax error
# - Shows simplified, best-practice messages (no long red JSON unless expanded)
...
$target = Join-Path $repo "app\admin\payouts\drivers\reports\page.tsx"

if (!(Test-Path (Split-Path $target -Parent))) {
  throw "Folder not found: $(Split-Path $target -Parent)`nAre you in the repo root?"
}
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div style={{ border: "1px solid #eee", borderRadius: 14, padding: 16 }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.5 }}>
          ...
          <div>• Auto-approve runs server-side with service role (no anon access).</div>
          <div>• Every payout change should be recorded in driver_payout_audit.</div>
          <div>• If you want, next step: add a run-log table for batch proof.</div>
        </div>
...
Write-Host "✅ Overwrote: $target" -ForegroundColor Green
Write-Host "Next: restart dev server (or it should hot-reload). If build error persists, run: npm run dev" -ForegroundColor Yellow
