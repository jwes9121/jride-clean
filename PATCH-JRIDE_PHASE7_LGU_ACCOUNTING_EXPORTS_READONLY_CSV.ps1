# PATCH-JRIDE_PHASE7_LGU_ACCOUNTING_EXPORTS_READONLY_CSV.ps1
# Phase 7: LGU / Accounting Exports (READ-ONLY, CSV)
#
# Creates:
# - API (GET only, Supabase REST, service role):
#   app/api/admin/reports/lgu-vendor/route.ts   -> reads admin_vendor_payout_monthly | admin_vendor_payout_summary
#   app/api/admin/reports/lgu-driver/route.ts   -> reads admin_driver_payout_daily_v1 | admin_driver_payout_requests_v1
#
# - UI (READ-ONLY report hub with CSV export):
#   app/admin/reports/lgu/page.tsx
#
# LOCKED:
# - NO wallet mutations
...
# --- Directories ---
$apiVendorDir = Join-Path $root "app\api\admin\reports\lgu-vendor"
$apiDriverDir = Join-Path $root "app\api\admin\reports\lgu-driver"
$uiDir        = Join-Path $root "app\admin\reports\lgu"

Ensure-Dir $apiVendorDir
Ensure-Dir $apiDriverDir
Ensure-Dir $uiDir

$apiVendorFile = Join-Path $apiVendorDir "route.ts"
$apiDriverFile = Join-Path $apiDriverDir "route.ts"
$uiFile        = Join-Path $uiDir "page.tsx"

Backup-IfExists $apiVendorFile
Backup-IfExists $apiDriverFile
Backup-IfExists $uiFile

# ============================================================
# API: Vendor LGU report
# GET /api/admin/reports/lgu-vendor?view=monthly|summary&vendor_id=...&month_start=YYYY-MM-01&limit=500
# Sources (EXISTING, NO CHANGES):
...
[System.IO.File]::WriteAllText($apiVendorFile, $apiVendorTs, $utf8NoBom)
Ok "[OK] Wrote: $apiVendorFile"

# ============================================================
# API: Driver LGU report
# GET /api/admin/reports/lgu-driver?view=daily|requests&driver_id=...&limit=500
# Sources (EXISTING, NO CHANGES):
# - admin_driver_payout_daily_v1
# - admin_driver_payout_requests_v1
#
# NOTE: We DO NOT assume date columns. This endpoint is read-only and returns rows.
# Filtering by driver_id is supported if column exists; otherwise returns unfiltered rows.
# ============================================================
$apiDriverTs = @'
import { NextResponse } from "next/server";
...
[System.IO.File]::WriteAllText($apiDriverFile, $apiDriverTs, $utf8NoBom)
Ok "[OK] Wrote: $apiDriverFile"

# ============================================================
# UI: LGU Report Hub (Vendor + Driver) with CSV export
# Route: /admin/reports/lgu
# - Vendor: monthly / summary (uses known vendor objects)
# - Driver: daily / requests (uses existing driver admin views)
# - Dynamic columns (NO assumptions)
# - Client-side "contains" search filter (works regardless of schema)
# - CSV export + optional totals for numeric columns
# ============================================================
$uiTsx = @'
"use client";
...
# Remove the accidental stray text line if present
# (We ensure the UI is clean ASCII-only and valid TSX.)
$uiTsx = $uiTsx -replace "^\s*clearly a safe, read-only export page for LGU/accounting\.\s*\r?\n", ""

[System.IO.File]::WriteAllText($uiFile, $uiTsx, $utf8NoBom)
Ok "[OK] Wrote: $uiFile"

Ok "[DONE] Phase 7 LGU/Accounting exports created (read-only + CSV)."
Ok "New UI: /admin/reports/lgu"
Ok "New APIs:"
Ok " - /api/admin/reports/lgu-vendor"
Ok " - /api/admin/reports/lgu-driver"
