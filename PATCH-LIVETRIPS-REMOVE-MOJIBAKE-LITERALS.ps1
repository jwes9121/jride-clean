...
# Replace any double-quoted string literal containing mojibake markers Ã or Â with "N/A"
# (display-only, safest possible fix)
$pattern = '"[^"\r\n]*[ÃÂ][^"\r\n]*"'
$matches = [regex]::Matches($txt, $pattern).Count
if ($matches -eq 0) {
  Fail "No mojibake string literals found (no Ã/Â inside double-quoted strings). Nothing to change."
}
