# PATCH-JRIDE_API_RIDES_LIST_USE_BOOKINGS_FALLBACK.ps1
# Minimal backend fix: /api/rides/list should NOT query non-existent table public.rides.
# Uses bookings first, falls back to rides if needed.
# Output stays compatible: { status:"ok", data:[...] }
...
$target = Join-Path $root "app\api\rides\list\route.ts"
