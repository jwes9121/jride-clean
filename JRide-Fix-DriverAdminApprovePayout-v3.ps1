...
Write-Host ""
Write-Host "JRide Fix: driver_admin_approve_payout() via Supabase CLI (linked)" -ForegroundColor Cyan
Write-Host "==================================================================" -ForegroundColor Cyan

$root = $PSScriptRoot
if (-not $root) { $root = (Get-Location).Path }

# Ensure Node/npm exist (CLI is run via npm exec)
...
# Your project ref (hard-coded by wrapper that wrote this file)
$projectRef = "gxaullwnxbkbjqbjotsr"

# SQL patch (DROP+CREATE to avoid return-type mismatch)
$sql = @"
begin;
...
try {
  Write-Host ""
  Write-Host "1) Supabase login (if not logged in yet)..." -ForegroundColor DarkCyan
  & npm exec --yes supabase@latest -- login

  Write-Host ""
  Write-Host "2) Linking repo to project: $projectRef" -ForegroundColor DarkCyan
  Push-Location $root
  & npm exec --yes supabase@latest -- link --project-ref $projectRef
  if ($LASTEXITCODE -ne 0) { Fail "supabase link failed." }

  Write-Host ""
  Write-Host "3) Running SQL patch via: supabase db query --file <temp.sql>" -ForegroundColor DarkCyan
  & npm exec --yes supabase@latest -- db query --file $tmpSql
  if ($LASTEXITCODE -ne 0) { Fail "supabase db query failed." }
