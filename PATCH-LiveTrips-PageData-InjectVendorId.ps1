# PATCH-LiveTrips-PageData-InjectVendorId.ps1
# Goal: Ensure LiveTrips page-data response includes vendor_id (and trip_type/driver_id) on each trip,
# so the Vendor wallet ledger modal stops saying "No vendor_id on this trip."
# Safe: makes a timestamp backup before overwriting the route file.

$ErrorActionPreference = "Stop"

function Fail($m) { throw $m }

# --- Sanity: run from repo root (where package.json exists) ---
$root = (Get-Location).Path
if (-not (Test-Path (Join-Path $root "package.json"))) {
  Fail "Run this from your repo root (where package.json is). Current: $root"
}

$rel = "app\api\admin\livetrips\page-data\route.ts"
...
$backupDir = Join-Path $root ("_patch_backups\{0}" -f $stamp)
...
Write-Host ("[1/3] Backup created: {0}" -f $backupPath) -ForegroundColor Cyan

# --- Overwrite route with a known-good implementation that:
# 1) calls admin_get_live_trips_page_data()
# 2) extracts trips even if the payload is array / {trips} / numeric keys
# 3) fetches bookings vendor_id/driver_id/trip_type by booking_code (and by id fallback)
# 4) injects vendor_id/driver_id/trip_type into each trip if missing
# 5) returns BOTH: numeric keys (0..n-1) + trips array + wallet balance maps + zones
$route = @'
...
[System.IO.File]::WriteAllText($path, $route, (New-Object System.Text.UTF8Encoding($false)))

Write-Host ("[2/3] Wrote: {0}" -f $rel) -ForegroundColor Green

# --- Quick syntax check: build just like you do ---
Write-Host "[3/3] Running: npm run build" -ForegroundColor Cyan
npm run build
if ($LASTEXITCODE -ne 0) { Fail "Build failed. Restore backup from: $backupPath" }

Write-Host "[OK] Build passed. Next: restart dev server and re-test Vendor wallet ledger on a trip that has vendor_id." -ForegroundColor Green
Write-Host "Tip: open a TAKEOUT trip you updated (vendor_id=1111...) -> Vendor tab should now load ledger rows." -ForegroundColor Yellow
