...
<#
REPLACE JRIDE WEB: app/verify/page.tsx with a clean, compiling version
PS5-safe, ASCII-only

Fixes:
- removes malformed fetch patch
- uses credentials: "include" on all verification fetches
- keeps same verification flow:
  1) GET /api/verify/session-user
  2) GET /api/public/passenger/verification/request
  3) POST /api/public/passenger/verification/upload
  4) POST /api/public/passenger/verification/request
#>
...
Write-Host "== REPLACE JRIDE WEB: clean verify page (V1 / PS5-safe) ==" -ForegroundColor Cyan

$root = (Resolve-Path -LiteralPath $ProjRoot).Path
Write-Host "Root: $root"

$target = Join-Path $root "app\verify\page.tsx"
...
$bak = BackupFile $target $bakDir "VERIFY_PAGE_CLEAN_V1"
