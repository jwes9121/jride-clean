...
export default function SmartAutoAssignSuggestions({
  ...
  const selectedTrip = useMemo(() => {
    ...
    return trips.find((t: any) => String(t.id ?? t.bookingId ?? t.booking_id ?? t.bookingCode ?? t.booking_code ?? "") === String(selectedTripId)) ?? null;
  ...
  const suggestions = useMemo(() => {
    ...
    // ✅ Trip zone resolution (prefer zone_id)
    ...
    return (drivers || [])
      .filter((d: any) => {
        ...
        // ✅ Block FULL zones (by zone_id)
        if (dzRow && String(dzRow.status).toUpperCase() === "FULL") return false;

        // ✅ Ordinance for passenger rides: must match zone_id if available
        if (!deliveryMode) {
          if (tripZoneId && driverZoneId) {
            return tripZoneId === driverZoneId;
          }
          // fallback to name if ids missing
      ...
      .map((d: any) => {
        ...
        const scoreRow = driverId ? driverScores[driverId] : null;
        const score0_100 = scoreRow?.score_0_100 ?? 70;

        const earnRow = driverId ? driverEarnings[driverId] : null;
        ...
        // fairness penalty (up to +10% when earnings high)
  ...
  async function assign(driverId: string) {
    ...
    // Your UI table "Code" often looks like UUID; we support both booking_code or uuid fallback in RPC
