...
import EarningsSummaryPanel from "./components/EarningsSummaryPanel";
...
export default function LiveTripsClient() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setDrivers(
          data.map((d: any) => ({
            id: d.driver_id,
            name: d.driver_id ? `Driver ${String(d.driver_id).slice(0, 4)}` : "Driver",
            lat: d.lat,
            lng: d.lng,
  ...
  const enhancedTrips = useMemo<LiveTrip[]>(() => {
    return liveTrips.map((t) => {
      ...
      return { ...(t as any), isProblem: stuckTripIds.has(id) || (t as any).isProblem } as LiveTrip;
    });
  ...
  useEffect(() => {
    ...
    if (!selectedTripId) {
      const first: any = enhancedTrips[0];
  ...
  const selectedTrip = useMemo(() => {
    ...
    return (
      enhancedTrips.find((t: any) => String(t.id ?? t.bookingCode ?? "") === selectedTripId) ?? null
    );
  }, [enhancedTrips, selectedTripId]);
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      {/* Earnings snapshot (no Mapbox changes) */}
      <EarningsSummaryPanel trips={enhancedTrips} />
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1">
          <LiveTripsMap trips={enhancedTrips} selectedTripId={selectedTripId} stuckTripIds={stuckTripIds} />
        </div>
      </div>
    </div>
  );
