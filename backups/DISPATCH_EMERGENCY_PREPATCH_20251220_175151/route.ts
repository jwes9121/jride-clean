...
export async function POST(req: Request) {
  try {
    ...
    // No is_emergency column assumptions. Just bump updated_at.
    ...
    return NextResponse.json({ ok: true, action: "emergency", bookingId: booking.id, bookingCode: booking.booking_code }, { status: 200 });
  } catch (e: any) {
    return NextResponse.json({ error: "EMERGENCY_UNEXPECTED", message: e?.message ?? "Unexpected error" }, { status: 500 });
  }
}