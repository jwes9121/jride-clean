...
export function LiveTripsMap({ trips, selectedTripId }: Props) {
  ...
  useEffect(() => {
    ...
    // ✅ Guard: Mapbox style may not be loaded yet (prevents getStyle() 'version' crash)
    if (!map.isStyleLoaded()) return;
    const __style = map.getStyle();
    if (!__style || !__style.layers) return;
    __style.layers?.forEach((l) => {
