...
function tripKey(t: any): string {
 if (!t) return "";
 const v =
 (t.booking_code ?? t.bookingCode ?? t.bookingcode ?? null) ??
 (t.uuid ?? t.id ?? null) ??
 "";
...
export default function LiveTripsClient() {
 ...
 // ------------------------------------------------------------
 // HARD refresh page-data so left list + right panel stay in sync
 // ------------------------------------------------------------
 const refreshPageData = async () => {
 try {
 const res = await fetch("/api/admin/livetrips/page-data?debug=1", { cache: "no-store" as any });
 const json: any = await res.json().catch(() => ({}));
 if (json && json.trips) { setAllTrips(json.trips); }
 if (json && (json.drivers || json.driverLocations)) { setDrivers(json.drivers || json.driverLocations); }
 setLastAction("OK: refreshed page-data");
 } catch (e: any) {
 setLastAction(String(e?.message || e));
 }
 };

 // Map/right-panel can request a refresh to keep UI consistent
 useEffect(() => {
 const handler = () => { refreshPageData().catch((e) => console.warn("refreshPageData failed", e)); };
 window.addEventListener("livetrips:refresh", handler as any);
 return () => window.removeEventListener("livetrips:refresh", handler as any);
 }, []);
 ...
 if (!selectedTripId || !ids.has(selectedTripId)) {
 setSelectedTripId(tripKey(out[0]) || null);
 }
 } else {
 setSelectedTripId(null);
 }
 ...
 loadPage().catch((e) => console.warn("loadPage failed", e));
...
try {
 ...
 const showThresholds = `Stuck watcher thresholds: on_the_way â‰¥ ${STUCK_THRESHOLDS_MIN.on_the_way} min, on_trip â‰¥ ${STUCK_THRESHOLDS_MIN.on_trip} min`;
 ...
 <div>on_the_way â‰¥ {STUCK_THRESHOLDS_MIN.on_the_way} min, on_trip â‰¥ {STUCK_THRESHOLDS_MIN.on_trip} min</div>
 </div>
 </div>
