...
function getZoneName(trip: any): string {
    // FOLLOW_DRIVER_ONLY_PATCH: pick best point to follow (driver -> pickup fallback)
  function getFollowLngLat(trip: any): { lng: number; lat: number } | null {
    if (!trip) return null;

    const dlng =
      trip.driver_lng ?? trip.driver_long ?? trip.driverLongitude ?? trip.driver_longitude ??
      trip.driverLocationLng ?? trip.driverLocationLong ?? trip.driver_location_lng ?? trip.driver_location_long;
    const dlat =
      trip.driver_lat ?? trip.driverLatitude ?? trip.driver_latitude ??
      trip.driverLocationLat ?? trip.driver_location_lat;

    if (typeof dlng === "number" && typeof dlat === "number") return { lng: dlng, lat: dlat };

    const plng =
      trip.pickup_lng ?? trip.from_lng ?? trip.from_long ?? trip.pickup_longitude ?? trip.from_longitude;
    const plat =
      trip.pickup_lat ?? trip.from_lat ?? trip.pickup_latitude ?? trip.from_latitude;

    if (typeof plng === "number" && typeof plat === "number") return { lng: plng, lat: plat };

    return null;
  }

  function followTripNow(trip: any) {
    const map = mapRef.current;
    if (!map) return;
    const pt = getFollowLngLat(trip);
    if (!pt) return;

    const now = Date.now();
    if (now - lastFollowAtRef.current < 700) return; // throttle to avoid jitter
    lastFollowAtRef.current = now;

    map.easeTo({ center: [pt.lng, pt.lat], duration: 650 });
  }

  // FOLLOW_DRIVER_ONLY_PATCH: when selection changes, re-enable follow and center once
  useEffect(() => {
    if (!mapReady) return;
    if (!selectedTrip) return;
    followRef.current = true;
    userInteractingRef.current = false;
    followTripNow(selectedTrip);
  }, [mapReady, trips]);

  // FOLLOW_DRIVER_ONLY_PATCH: keep following while enabled
  useEffect(() => {
    if (!mapReady) return;
    if (!selectedTrip) return;
    ...
    const t = setInterval(() => {
      ...
      // FOLLOW_DRIVER_ONLY_PATCH_ROBUST
  function findTripForFollow(): any | null {
    if (!selectedTripId) return null;
    const sid = String(selectedTripId);
    for (const t of (trips || []) as any[]) {
      const id =
        String(t?.id ?? t?.booking_id ?? t?.bookingId ?? t?.booking_code ?? t?.bookingCode ?? "");
      if (id && id === sid) return t;
    }
    // fallback: sometimes selectedTripId is booking_code while list uses id
    for (const t of (trips || []) as any[]) {
      const code = String(t?.booking_code ?? t?.bookingCode ?? "");
      if (code && code === sid) return t;
    }
    return null;
  }
  ...
  function getFollowLngLat(trip: any): { lng: number; lat: number } | null {
    ...
    const dlng =
      trip.driver_lng ?? trip.driver_long ?? trip.driverLongitude ?? trip.driver_longitude ??
      trip.driver_location_lng ?? trip.driver_location_long ?? trip.driverLocationLng ?? trip.driverLocationLong;
    const dlat =
      trip.driver_lat ?? trip.driverLatitude ?? trip.driver_latitude ??
      trip.driver_location_lat ?? trip.driverLocationLat;

    if (typeof dlng === "number" && typeof dlat === "number") return { lng: dlng, lat: dlat };

    const plng =
      trip.pickup_lng ?? trip.from_lng ?? trip.from_long ?? trip.pickup_longitude ?? trip.from_longitude ??
      trip.pickupLng ?? trip.fromLng;
    const plat =
      trip.pickup_lat ?? trip.from_lat ?? trip.pickup_latitude ?? trip.from_latitude ??
      trip.pickupLat ?? trip.fromLat;
  ...
  function followTripNow(trip: any) {
    ...
    const now = Date.now();
    if (now - lastFollowAtRef.current < 700) return; // throttle
    lastFollowAtRef.current = now;

    map.easeTo({ center: [pt.lng, pt.lat], duration: 650 });
  }

  // On selection change: re-enable follow + center once
  useEffect(() => {
    if (!mapReady) return;
    const trip = findTripForFollow();
    if (!trip) return;
    followRef.current = true;
    userInteractingRef.current = false;
    followTripNow(trip);
  ...
  }, [mapReady, trips, selectedTripId]);

return () => clearInterval(t);
  }, [mapReady, selectedTrip]);
...
export const LiveTripsMap: React.FC<LiveTripsMapProps> = ({
  ...
  useEffect(() => {
    ...
    // FOLLOW_DRIVER_ONLY_PATCH: stop following if user manually moves the map
    const stopFollow__dup = () => { userInteractingRef.current = true; followRef.current = false; };
    map.on("dragstart", stopFollow__dup);
    map.on("zoomstart", stopFollow__dup);
    map.on("rotatestart", stopFollow__dup);
    map.on("pitchstart", stopFollow__dup);
  ...
  return (
    <>
      ...
      {/* FOLLOW_BTN_ONLY_PATCH */}
      <div className="absolute top-3 left-3 z-30 pointer-events-auto">
        <button
          className="rounded bg-white/90 px-3 py-2 text-xs font-semibold shadow border hover:bg-white"
          onClick={() => {
            const map = mapRef?.current;
            if (!map) return;
            // Re-center to whatever is currently selected
            try {
              // If selectedTrip exists and helper funcs exist, use them; otherwise no-op.
              // This button is intentionally safe and will not crash.
              // @ts-ignore
              const raw = typeof selectedTrip !== "undefined" ? selectedTrip : null;
              // @ts-ignore
              const getDriverRealFn = typeof getDriverReal === "function" ? getDriverReal : null;
              // @ts-ignore
              const getPickupFn = typeof getPickup === "function" ? getPickup : null;
              // @ts-ignore
              const getDropoffFn = typeof getDropoff === "function" ? getDropoff : null;

              // If your file uses LngLatTuple, these helpers usually return [lng,lat]
              const target =
                (getDriverRealFn && raw ? getDriverRealFn(raw) : null) ||
                (getDropoffFn && raw ? getDropoffFn(raw) : null) ||
                (getPickupFn && raw ? getPickupFn(raw) : null);
          ...
          title="Re-center to selected trip/driver"
        >
          Follow
        </button>
      </div>
                ...
                >
                  ...
                  {s.distanceMeters != null && (
                    <div className="text-[10px] text-slate-500">
                      ~{(s.distanceMeters / 1000).toFixed(2)} km away ÃƒÆ’Ã†'Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢â‚¬Å¡Ã‚¬Ãƒâ€¦Ã‚¡ÃƒÆ’Ã†'Ãƒ¢Ã¢â€š¬Ã…¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚·{" "}
                      {s.reason}
                    </div>
                  )}
                </li>
            ...
            <div className="space-y-1 text-[11px] text-slate-700">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  ...
                  {selectedOverview.isStuck ? " ÃƒÆ’Ã†'Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢â‚¬Å¡Ã‚¬Ãƒâ€¦Ã‚¡ÃƒÆ’Ã†'Ãƒ¢Ã¢â€š¬Ã…¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚· STUCK" : ""}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {selectedOverview.lastUpdate
                    ...
                    : "ÃƒÆ’Ã†'Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚¢ÃƒÆ’Ã†'Ãƒâ€šÃ‚¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢â‚¬Å¡Ã‚¬Ãƒâ€¦Ã‚¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚¬ÃƒÆ’Ã†'Ãƒâ€šÃ‚¢ÃƒÆ’Ã‚¢Ãƒ¢Ã¢â€š¬Ã…¡Ãƒâ€šÃ‚¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚"}
                </span>
              </div>
            </div>
