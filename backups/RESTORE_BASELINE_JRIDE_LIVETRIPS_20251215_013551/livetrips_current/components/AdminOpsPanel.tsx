...
type Props = {
  ...
  zoneCaps?: ZoneCapRow[]; // ✅ optional safe
  driverScores?: Record<string, DriverScoreRow>; // ✅ optional safe
  driverEarnings?: Record<string, DriverEarningsRow>; // ✅ optional safe
};
...
export default function AdminOpsPanel(props: Props) {
  const trips = props.trips || [];
  const selectedTripId = props.selectedTripId ?? null;
  const onSelectTrip = props.onSelectTrip;

  // ✅ HARD safety defaults (prevents your crash)
      ...
      <div className="border-b bg-slate-50 p-2">
        ...
        {zoneCards.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {zoneCards.map((z) => (
              <div key={String(z.zone_id)} className="rounded border bg-white px-2 py-1">
