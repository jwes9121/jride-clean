...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase
      .from("bookings")
      .select("id, booking_code, passenger_name, total_service_fee, vendor_status, created_at, service_type")
    ...
    return NextResponse.json({
      orders: (data ?? []).map((o) => ({
        ...
        total_bill: o.total_service_fee       // ✅ uses your real fare field
      }))
    });
  ...
  try {
    ...
    return NextResponse.json({
      order: {
        ...
        total_bill: data.total_service_fee
      }
    });
