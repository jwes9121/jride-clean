cd C:\Users\jwes9\Desktop\jride-clean-fresh
...
$bakDir = Join-Path $root ("backups\LIVETRIPS_STUCKTEXT_PATCH_" + $stamp)
New-Item -ItemType Directory -Force -Path $bakDir | Out-Null

# Needles (ASCII only)
$needles = @("Stuck watcher thresholds","on_the_way","on_trip")

Write-Host "[1/3] Searching for stuck watcher text sources..." -ForegroundColor Cyan
$hitsAll = @()
foreach ($n in $needles) {
  $hits = Get-ChildItem $scanDir -Recurse -Include *.ts,*.tsx -File |
    Select-String -SimpleMatch -Pattern $n -ErrorAction SilentlyContinue
  if ($hits) { $hitsAll += $hits }
}

$files = $hitsAll | Select-Object -ExpandProperty Path -Unique
if ($files.Count -eq 0) { throw "No matches found under $scanDir" }
...
# E2 80 93 => "â€“"  (U+00E2 U+20AC U+201C)
...
foreach ($f in $files) {
  ...
  # Normalize known dash/ellipsis variants
  $txt = $txt.Replace($mojiEndash, "-")
  $txt = $txt.Replace($mojiEmdash, "-")
  $txt = $txt.Replace($enDash, "-")
  $txt = $txt.Replace($emDash, "-")
...
'@ | Set-Content -Encoding Ascii .\PATCH-FindAndFix-StuckThresholdText-ASCII.ps1

powershell -ExecutionPolicy Bypass -File .\PATCH-FindAndFix-StuckThresholdText-ASCII.ps1
