# PATCH-JRIDE_DRIVER_LOCATION_PING_COORD_FALLBACK_V1_PS5SAFE.ps1
# Replaces: app/api/driver/location/ping/route.ts
# Goal:
# - Fix driver_locations upsert when lat/lng are NOT NULL
# - Reuse previous lat/lng when status-only ping arrives
# - Fail cleanly if first-ever ping has no coords
# - Preserve device lock logic
# - Preserve edge-trigger retry logic
...
$bak = Join-Path $bakDir ("route.ts.bak.DRIVER_LOCATION_PING_COORD_FALLBACK_V1.{0}" -f $stamp)
...
export async function POST(req: Request) {
  try {
    ...
    return json(200, {
      ...
      active_device_id: (lock as any).active_device_id,
      coords_source: hasIncomingCoords ? "incoming" : "previous_row",
      lat: finalLat,
      lng: finalLng,
    });
  } catch (e: any) {
...
Write-Info "Changed:"
Write-Host " - Reuses previous lat/lng for status-only pings"
Write-Host " - Returns clean 400 if no incoming coords and no previous location row"
Write-Host " - Preserves edge-trigger retry"
Write-Host " - Preserves device lock behavior"
Write-Host " - Includes row id in upsert when previous row exists"