# PATCH-JRIDE_PASSENGER_UI_PHASE_P2_TRIP_RECEIPT_UI_ONLY_V2.ps1
...
function Insert-After-Line($text, $needle, $insert, $label){
  ...
  $lineEnd = $text.IndexOf("`n", $p)
  if($lineEnd -lt 0){ $lineEnd = $text.Length }
  return $text.Substring(0, $lineEnd + 1) + $insert + $text.Substring($lineEnd + 1)
}
...
if($t.IndexOf("PHASE P2: Trip receipt (terminal-only, UI-only)") -ge 0){
  Write-Host "[OK] PHASE P2 receipt already present (skip)"
...
# Primary anchor: P1 stepper call (very stable)
$needle = "{p1RenderStepper(liveStatus)}"
if($t.IndexOf($needle) -lt 0){
  # fallback to the raw function call if braces differ
  $needle = "p1RenderStepper(liveStatus)"
  if($t.IndexOf($needle) -lt 0){
    Fail "Could not find P1 stepper anchor (p1RenderStepper(liveStatus)). PHASE P1 must exist on ride page."
  }
}
...
$t = Insert-After-Line $t $needle $receiptBlock "P1 stepper call"
