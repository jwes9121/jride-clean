# PATCH-DISPATCH-LGU-FIXER-AND-EXCEL.ps1
# - Full file replace: app/dispatch/page.tsx
# - Create new route: app/api/dispatch/lgu/route.ts
# Adds:
#  (2) LGU Fields Fixer (write from_label/to_label/verified_fare + distance_km into passenger_fare_response)
#  (3) Download LGU Excel (multi-sheet SpreadsheetML, no deps)
...
$lguRouteDir = "app\api\dispatch\lgu"
$lguRoute = Join-Path $lguRouteDir "route.ts"

if (!(Test-Path $dispatch)) { Fail "Missing file: $dispatch" }

Copy-Item $dispatch "$dispatch.bak.$ts" -Force
Write-Host "[OK] Backup: $dispatch.bak.$ts" -ForegroundColor Green

EnsureDir $lguRouteDir

# --- New API route: /api/dispatch/lgu (token-gated, no auth loops)
# Set on Vercel: DISPATCH_ADMIN_TOKEN = (any long secret)
# Client sends header x-dispatch-admin-token
$lguRouteContent = @'
...
WriteUtf8NoBom $lguRoute $lguRouteContent
Write-Host "[DONE] Wrote: $lguRoute" -ForegroundColor Green

# --- Dispatch page: add LGU fixer modal + Excel multi-sheet export
$dispatchContent = @'
"use client";
...
WriteUtf8NoBom $dispatch $dispatchContent
Write-Host "[DONE] Patched: $dispatch" -ForegroundColor Green

Write-Host ""
Write-Host "NEXT STEPS:" -ForegroundColor Yellow
Write-Host "1) npm.cmd run build" -ForegroundColor Yellow
Write-Host "2) npm.cmd run dev" -ForegroundColor Yellow
Write-Host "3) Set Vercel env DISPATCH_ADMIN_TOKEN (required for LGU Fix writes)" -ForegroundColor Yellow
