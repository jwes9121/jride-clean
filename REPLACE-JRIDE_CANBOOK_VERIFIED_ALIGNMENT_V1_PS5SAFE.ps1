...
$target = Join-Path $ProjRoot "app\api\public\passenger\can-book\route.ts"
if (!(Test-Path $target)) {
  throw "Target route not found: $target"
}

$bakDir = Join-Path $ProjRoot "_patch_bak"
if (!(Test-Path $bakDir)) { New-Item -ItemType Directory -Path $bakDir | Out-Null }

$bak = Join-Path $bakDir ("route.ts.bak.CANBOOK_VERIFIED_ALIGN_V1." + (Stamp))
...
async function computeVerified(supabase: any, user: any): Promise<{ verified: boolean; verification_status: string | null; source: string | null }> {
  ...
  // 3) Fallback: passengers flags (best-effort; don’t assume schema)
  try {
...
Write-Host "== JRIDE Replace: can-book route verified alignment (V1 / PS5-safe) =="
Write-Host "[OK] Target: $target"
Write-Host "[OK] Backup: $bak"
Write-Host "[OK] Wrote clean can-book route that aligns verified with passenger_verifications.status + legacy requests + passengers fallback."