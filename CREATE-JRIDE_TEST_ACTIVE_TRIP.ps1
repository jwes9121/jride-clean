# CREATE-JRIDE_TEST_ACTIVE_TRIP.ps1
# Generates a SQL script you can run in Supabase SQL Editor to create ONE active booking in public.bookings
# WITHOUT assuming columns exist (dynamic inspection).
#
# Output file: .\CREATE_TEST_ACTIVE_TRIP.sql
...
$out = "CREATE_TEST_ACTIVE_TRIP.sql"
...
BEGIN
  ...
  BEGIN
    ...
    RAISE NOTICE '✅ Created test active trip. booking_code=%', out_code;
  EXCEPTION WHEN others THEN
    RAISE NOTICE '❌ Insert failed. SQL=%', ins_sql;
    RAISE;
  END;
END
\$\$;
...
Set-Content -LiteralPath $out -Value $sql -Encoding UTF8
Ok "Wrote $out"
Ok "Next: open Supabase SQL Editor, paste contents of $out, run it, then copy the printed booking_code."
