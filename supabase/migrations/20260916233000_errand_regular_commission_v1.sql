-- JRide regular Errand commission v1.
-- Prospective from 2026-09-17 00:00 Asia/Manila (2026-09-16 16:00:00Z).
-- Customer Errand pricing components remain unchanged.
-- New Errands use a whole-peso JRide cut of 25% of final fare, capped at PHP 20.
...
begin
  ...
  if new.service_type = 'errand' then
    ...
    if v_regular_commission_enabled then
      v_company := least(round(v_total * 0.25, 0), 20);
    else
      v_company := 20;
    end if;
...
comment on function public.apply_service_pricing() is
  'JRide service pricing trigger. New Errands created on or after 2026-09-17 00:00 Asia/Manila use a whole-peso 25 percent company cut capped at PHP 20; older Errands retain the historical fixed PHP 20 cut. Errand night pricing is disabled pending separate approval. Takeout pricing is unchanged.';

comment on function public.apply_errand_pricing() is
  'Legacy Errand pricing helper aligned to the Sept 4 absorbed-approach model and the prospective regular Errand commission policy. Errand night pricing is disabled pending separate approval.';
