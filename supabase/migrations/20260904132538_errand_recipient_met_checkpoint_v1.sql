alter table public.errand_jobs
  add column if not exists final_recipient_met_at timestamptz;

create or replace function public.errand_driver_mark_recipient_met_v1(
  p_booking_id uuid,
  p_driver_id uuid
)
returns jsonb
...
begin
  ...
  if v_booking.status <> 'on_trip' then
    return jsonb_build_object(
      'ok', false,
      'error', 'ERRAND_RECIPIENT_MET_STATUS_INVALID',
  ...
  if v_job.handoff_completed_at is not null then
    return jsonb_build_object('ok', false, 'error', 'ERRAND_HANDOFF_ALREADY_COMPLETED');
  end if;
  ...
  if v_job.final_recipient_met_at is not null
     and v_job.errand_stage = 'final_recipient_met'
     and v_job.waiting_started_at is null then
    return jsonb_build_object(
      'ok', true,
      'already_met', true,
      ...
      'waiting_accumulated_seconds', greatest(coalesce(v_job.waiting_accumulated_seconds, 0), 0),
      'waiting_minutes', greatest(coalesce(v_booking.waiting_minutes, 0), 0),
      'waiting_fee', greatest(coalesce(v_booking.waiting_fee, 0), 0),
      'total_errand_fare', greatest(coalesce(v_booking.total_errand_fare, 0), 0)
    );
  end if;

  if v_job.errand_stage <> 'waiting_at_final_handoff'
     or v_job.waiting_started_at is null then
    return jsonb_build_object(
      'ok', false,
      'error', 'FINAL_HANDOFF_ARRIVAL_REQUIRED',
      'errand_stage', v_job.errand_stage
    );
  end if;

  v_elapsed := greatest(
    floor(extract(epoch from (now() - v_job.waiting_started_at)))::integer,
    0
  );
  ...
  if v_elapsed >= 1800 then
    return jsonb_build_object(
      'ok', false,
      'error', 'FINAL_HANDOFF_WAIT_LIMIT_REACHED',
      'local_wait_seconds', v_elapsed,
      'local_wait_limit_seconds', 1800,
      'required_action', 'escalate_unreachable'
    );
  end if;

  v_job_wait := greatest(coalesce(v_job.waiting_accumulated_seconds, 0), 0) + v_elapsed;
  v_wait_minutes := case
  ...
  set waiting_accumulated_seconds = v_job_wait,
      ...
      final_recipient_met_at = now(),
      errand_stage = 'final_recipient_met',
      updated_at = now()
  where booking_id = p_booking_id;
  ...
  return jsonb_build_object(
    ...
    'errand_stage', 'final_recipient_met',
    'final_recipient_met_at', now(),
    'final_wait_seconds_charged', v_elapsed,
...
revoke all on function public.errand_driver_mark_recipient_met_v1(uuid, uuid) from public;
revoke all on function public.errand_driver_mark_recipient_met_v1(uuid, uuid) from anon;
revoke all on function public.errand_driver_mark_recipient_met_v1(uuid, uuid) from authenticated;
grant execute on function public.errand_driver_mark_recipient_met_v1(uuid, uuid) to service_role;
...
begin
  ...
  if v_job.errand_stage = 'waiting_at_final_handoff'
     and v_job.waiting_started_at is not null then
    v_elapsed := greatest(
      floor(extract(epoch from (now() - v_job.waiting_started_at)))::integer,
      0
    );
  ...
  elsif v_job.errand_stage = 'final_recipient_met'
        and v_job.final_recipient_met_at is not null
        and v_job.waiting_started_at is null then
    v_elapsed := 0;
  else
    return jsonb_build_object(
  ...
  if v_job.is_pabili then
    ...
    if v_change_remaining > 0.009 then
      insert into public.errand_pabili_fund_events (
        ...
        'Remaining customer change handed over with item and physical receipt at final handoff'
      );
    end if;
  ...
  return jsonb_build_object(
    ...
    'status', v_after.status,
    'errand_stage', 'handoff_complete',
    'final_handoff_wait_seconds', v_elapsed,
    'waiting_accumulated_seconds', v_job_wait,
    'waiting_minutes', v_after.waiting_minutes,
    'waiting_fee', v_after.waiting_fee,
...
begin
  ...
  if not (
    (v_job.errand_stage = 'waiting_at_final_handoff' and v_job.waiting_started_at is not null)
    or
    (v_job.errand_stage = 'final_recipient_met' and v_job.final_recipient_met_at is not null and v_job.waiting_started_at is null)
  ) then
    return jsonb_build_object('ok', false, 'error', 'FINAL_HANDOFF_ARRIVAL_REQUIRED');
  end if;
  ...
  if p_amount > 0 then
    insert into public.errand_pabili_fund_events (
      ...
      p_amount,
      'in_person',
      'Customer change returned at final handoff'
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'change_remaining', 0,
    'waiting_continues', v_job.errand_stage = 'waiting_at_final_handoff'
  );
end;
$function$;
