drop function if exists public.agrimarket_create_reserved_order_v1(uuid, uuid, jsonb, numeric, numeric);
drop function if exists public.agrimarket_producer_decide_order_v1(text, uuid, text, text, timestamptz);
drop function if exists public.agrimarket_expire_pending_orders_v1(timestamptz, integer);
...
create function public.agrimarket_create_reserved_order_v1(
...
create function public.agrimarket_producer_decide_order_v1(
  p_order_code text,
...
begin
  ...
  if v_order.producer_confirm_expires_at <= p_now then
    ...
    set status = 'producer_timeout',
        producer_responded_at = p_now,
        producer_timeout_at = p_now,
        updated_at = p_now
    where o.id = v_order.id;
    ...
    update public.agrimarket_orders o
    set status = 'producer_accepted',
        producer_responded_at = p_now,
        producer_accepted_at = p_now,
        updated_at = p_now
    where o.id = v_order.id;
...
create function public.agrimarket_expire_pending_orders_v1(
  p_now timestamptz default clock_timestamp(),
...
begin
  for v_order in
    ...
    from public.agrimarket_orders o
    where o.status = 'awaiting_producer'
      and o.producer_confirm_expires_at <= p_now
    order by o.producer_confirm_expires_at, o.id
    limit v_limit
  ...
  loop
    ...
    set status = 'producer_timeout',
        ...
        updated_at = p_now
    where o.id = v_order.id
      and o.status = 'awaiting_producer';
