# PATCH-DISPATCH-DATERANGE-FILTER.ps1
# Full file replace: app/dispatch/page.tsx
# Adds date range filter for LGU exports and LGU row counts:
# - Today
# - This week (Mon..Sun, local time)
# - Custom (start/end date)
# No DB/schema changes. ASCII-only.
...
# Replace the whole file with a known-good version that includes:
# - LGU CSV export
# - LGU Excel export (SpreadsheetML multi-sheet)
# - SUMMARY sheet (if your current file already has it, this keeps it)
# - LGU Fix modal + token
...
function endOfWeekSunLocal() {
  const s = startOfWeekMonLocal();
  const e = new Date(s);
...
function inRange(iso: string | null | undefined, start: Date | null, end: Date | null) {
  if (!iso) return false;
  const d = new Date(iso);
...
export default function DispatchPage() {
  ...
  const [customStart, setCustomStart] = useState<string>(() => ymdLocal(new Date()));
  ...
  const rangeStartEnd = useMemo(() => {
    if (rangeMode === "today") {
      ...
      const e = new Date(s);
      e.setHours(23, 59, 59, 999);
      return { start: s, end: e, label: ymdLocal(new Date()) };
    }

    if (rangeMode === "week") {
      const s = startOfWeekMonLocal();
      const e = endOfWeekSunLocal();
      return { start: s, end: e, label: `${ymdLocal(s)}_to_${ymdLocal(e)}` };
    }

    // custom
    const s = customStart ? new Date(customStart + "T00:00:00") : null;
    const e = customEnd ? new Date(customEnd + "T23:59:59") : null;
    return { start: s, end: e, label: `${customStart || "NA"}_to_${customEnd || "NA"}` };
  ...
  const rowsForExport = useMemo(() => {
    ...
    return rowsSorted.filter((b) => {
      ...
      // Date range filter
      if (!inRange(b.created_at || null, rangeStartEnd.start, rangeStartEnd.end)) return false;

      return true;
    });
  }, [rowsSorted, muniFilter, completedOnly, rangeStartEnd]);
  ...
  function exportLguCsv() {
    ...
    const filename = `JRIDE_LGU_TRIP_LEDGER_${ymd}_${muni}_${rangeStartEnd.label}.csv`;
  ...
  function exportLguExcel() {
    ...
    const filename = `JRIDE_LGU_TRIP_LEDGER_${ymd}_MULTI-SHEET_${rangeStartEnd.label}.xls`;
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="rounded border p-3 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          title="Downloads Excel multi-sheet file (SUMMARY + ALL + per municipality)"
        >
          Download LGU Excel
        </button>
      </div>
              ...
              obs.map((a: any) => (
                <div key={a.id} className="rounded border bg-white p-2">
                  ...
                  <div className="mt-1 text-[11px] text-slate-600">
                    {String(a.type || "status")} -> {String(a.nextStatus || a.driverId || "-")} ({a.ok ? "OK" : "BLOCKED"})
                  </div>
                </div>
              ))
            )}
...
Write-Host "[DONE] Wrote date range filter build: $target" -ForegroundColor Green
