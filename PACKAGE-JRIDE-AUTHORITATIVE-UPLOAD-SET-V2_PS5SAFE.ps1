...
if ($badFiles.Count -gt 0) {
    foreach ($f in $badFiles) {
        $relBad = $f.FullName.Substring($bundleRoot.Length).TrimStart('\','/')
        Write-Host ("[REMOVE] " + $relBad)
        Remove-Item -LiteralPath $f.FullName -Force
        $manifestLines.Add("[REMOVED] $relBad") | Out-Null
        Add-ManifestRow -Rows $manifestRows -Category "sanity_check" -RelativePath $relBad -Status "REMOVED"
    }
} else {
    ...
    Add-ManifestRow -Rows $manifestRows -Category "sanity_check" -RelativePath "" -Status "OK_NO_BACKUPS_FOUND"
}
...
Write-Host "Upload the ZIP file or the contents of upload_bundle."
Write-Host "This package contains only the authoritative target files."
Write-Host "No project source files were modified."