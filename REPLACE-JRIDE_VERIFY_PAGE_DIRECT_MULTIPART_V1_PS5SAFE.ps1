...
<#
REPLACE JRIDE WEB: app/verify/page.tsx with direct multipart submit version
PS5-safe, ASCII-only

Fixes:
- removes dependency on broken /api/public/passenger/verification/upload route
- submits files directly to /api/public/passenger/verification/request as multipart/form-data
- keeps same session-user + request GET flow
#>
...
Write-Host "== REPLACE JRIDE WEB: verify page direct multipart submit (V1 / PS5-safe) ==" -ForegroundColor Cyan
...
$bak = BackupFile $target $bakDir "VERIFY_PAGE_DIRECT_MULTIPART_V1"
