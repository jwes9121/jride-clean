# PATCH-JRIDE_PHASE6G_STATUS_LIFECYCLE_HARDEN.ps1
# Phase 6G: status lifecycle hardening (requested -> assigned) + prevent reassignment overwrite
# Files:
...
# -------------------------
# app/api/dispatch/assign/route.ts (overwrite with hardened version)
# -------------------------
$assignTs = @'
...
# -------------------------
# app/api/public/passenger/book/route.ts
# Patch: after assignment update, re-read booking and return final booking (status/driver_id)
# -------------------------
$bookTxt = Get-Content $bookRoute -Raw

if ($bookTxt -notmatch "return NextResponse\.json") { throw "Unexpected book route structure: $bookRoute" }

# Insert a re-read helper if missing, and ensure response includes final booking after assign update.
# We'll do a conservative replace:
# Find "assign = { ok: true" blocks and after update, we will re-read booking by id (best-effort) and return that booking in response.

$rx = [regex]'(?s)(const\s+upd\s*=\s*await\s+bestEffortUpdateBooking\([^\)]*\)\s*;\s*\r?\n\s*assign\s*=\s*\{\s*ok:\s*true,[^\}]*\}\s*;\s*)'
$bookTxt2 = $rx.Replace($bookTxt, '$1' + "`r`n" + @'
        // Phase 6G: re-read booking so API returns final status/driver_id (best-effort)
        try {
          const reread = await supabase.from("bookings").select("*").eq("id", String(booking.id)).maybeSingle();
          if (!reread.error && reread.data) {
            booking = reread.data as any;
          }
        } catch {}
'@)

# If no replacement happened, leave file as-is (still works); we won't break builds.
if ($bookTxt2 -eq $bookTxt) {
  Write-Host "[WARN] Could not auto-insert reread block in book route. Leaving book route unchanged."
} else {
  WriteUtf8NoBom $bookRoute $bookTxt2
  Write-Host "[OK] Patched (reread booking after assign): $bookRoute"
}
