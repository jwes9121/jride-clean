...
Write-Host ""
Write-Host "JRide Fix: driver_admin_approve_payout() (NO MANUAL EDITS)" -ForegroundColor Cyan
...
# Prefer explicit DB URL if you have it
$dbUrl = FirstNonEmpty @(
  $envMap["SUPABASE_DB_URL"],
  $envMap["DATABASE_URL"],
  $env:SUPABASE_DB_URL,
  $env:DATABASE_URL
)

# Supabase CLI project ref fallback (optional)
$projectRef = FirstNonEmpty @(
  $envMap["SUPABASE_PROJECT_REF"],
  $env:SUPABASE_PROJECT_REF
)

# SQL fix (drop+create to avoid 42P13)
$sql = @"
begin;
...
begin
  ...
  if v_driver.wallet_balance is null then
    raise exception 'Driver wallet_balance is null (driver_id=%)', v_driver.id;
  end if;

  if v_driver.min_wallet_required is null then
    raise exception 'Driver min_wallet_required is null (driver_id=%)', v_driver.id;
  end if;
  ...
  return jsonb_build_object(
    ...
    'amount', v_req.amount,
    'wallet_before', v_driver.wallet_balance,
    'wallet_after', v_new_wallet,
    'status', 'paid'
  );
end;
\$\$;
...
$tmpSql = Join-Path $env:TEMP ("jride_fix_driver_admin_approve_payout_" + [guid]::NewGuid().ToString("n") + ".sql")
$sql | Out-File -FilePath $tmpSql -Encoding UTF8

try {
  $psql = Find-Command "psql"
  $supabase = Find-Command "supabase"

  if ($dbUrl -and $psql) {
    Write-Host ""
    Write-Host "Using psql via SUPABASE_DB_URL / DATABASE_URL..." -ForegroundColor DarkCyan
    Write-Host "Running SQL patch (DROP+CREATE function)..." -ForegroundColor DarkCyan

    # IMPORTANT: do not echo the URL (contains password)
    & $psql $dbUrl -v "ON_ERROR_STOP=1" -f $tmpSql
    if ($LASTEXITCODE -ne 0) { Fail "psql failed with exit code $LASTEXITCODE" }

    Write-Host ""
    Write-Host "✅ Patch applied successfully via psql." -ForegroundColor Green
    exit 0
  }

  if ($projectRef -and $supabase) {
    Write-Host ""
    Write-Host "psql/DB URL not available. Trying Supabase CLI using SUPABASE_PROJECT_REF..." -ForegroundColor DarkCyan
    Write-Host "Running SQL patch..." -ForegroundColor DarkCyan

    # This requires: `supabase login` already done in this machine
    & $supabase db execute --project-ref $projectRef --file $tmpSql
    if ($LASTEXITCODE -ne 0) { Fail "supabase db execute failed with exit code $LASTEXITCODE" }

    Write-Host ""
    Write-Host "✅ Patch applied successfully via Supabase CLI." -ForegroundColor Green
    exit 0
  }

  Write-Host ""
  Write-Host "❌ Cannot apply patch automatically yet." -ForegroundColor Red
  Write-Host ""
  Write-Host "To run this fully from PowerShell, you need ONE of these:" -ForegroundColor Yellow
  Write-Host "  A) psql installed + SUPABASE_DB_URL (or DATABASE_URL) in .env.local" -ForegroundColor Yellow
  Write-Host "  B) Supabase CLI installed + SUPABASE_PROJECT_REF in .env.local (and run: supabase login)" -ForegroundColor Yellow
  Write-Host ""
  Write-Host "Your current script failed because PostgREST cannot run CREATE/DROP FUNCTION." -ForegroundColor DarkGray
