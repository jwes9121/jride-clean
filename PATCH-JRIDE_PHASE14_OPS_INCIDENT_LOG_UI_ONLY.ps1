...
$incidentsPage = Join-Path $root "app\admin\ops\incidents\page.tsx"

if (!(Test-Path $controlCenter)) { Fail "Missing file: $controlCenter" }

Backup-IfExists $controlCenter
Backup-IfExists $incidentsPage

# ---------------------------------------------------------------------
# 1) Patch Control Center: insert "Incident Log (Read-only)" after Auto-Assign Monitor item
#    Anchor-based (no section heading dependency).
# ---------------------------------------------------------------------
$cc = Get-Content -Raw -Path $controlCenter

if ($cc -match 'href\s*:\s*["'']\/admin\/ops\/incidents["'']') {
  Write-Host "[OK] Control Center already has /admin/ops/incidents link; skipping insert."
} else {
...
'@
  ...
  # Prefer insert after Auto-Assign Monitor item (exists in Ops area)
  ...
  if ([regex]::IsMatch($cc, $afterAutoAssign)) {
    $cc = [regex]::Replace($cc, $afterAutoAssign, ('$1' + $incidentItem), 1)
    Write-Host "[OK] Inserted Incident Log after Auto-Assign Monitor."
  } else {
    # Next best anchor: insert before Stuck Drivers item if present
    $beforeStuck = '(?s)(\{\s*title\s*:\s*"Stuck Drivers".*?href\s*:\s*["'']\/admin\/ops\/stuck-drivers["'']\s*,\s*\}\s*,?\s*)'
    if ([regex]::IsMatch($cc, $beforeStuck)) {
      $cc = [regex]::Replace($cc, $beforeStuck, ($incidentItem + '$1'), 1)
      Write-Host "[OK] Inserted Incident Log before Stuck Drivers."
    } else {
      # Last resort: insert before Audit Trail if exists (still ops-ish)
      $beforeAudit = '(?s)(\{\s*title\s*:\s*"Audit Trail \(Read-only\)".*?href\s*:\s*["'']\/admin\/audit["'']\s*,\s*\}\s*,?\s*)'
      if ([regex]::IsMatch($cc, $beforeAudit)) {
        $cc = [regex]::Replace($cc, $beforeAudit, ($incidentItem + '$1'), 1)
        Write-Host "[OK] Inserted Incident Log before Audit Trail."
      } else {
        Fail "Could not find insertion anchor (Auto-Assign / Stuck Drivers / Audit). Paste app/admin/control-center/page.tsx so we can patch deterministically."
      }
    }
  }
}
...
# ---------------------------------------------------------------------
# 2) Create /admin/ops/incidents page (READ-ONLY, click-to-load GET only)
#    Data source: GET /api/admin/livetrips/page-data
#    Buckets:
#      - Stuck/Problem
#      - Unassigned too long (requested + no driver + age >= threshold)
#      - At-risk (flag or status)
#      - Cancelled recent (in range)
#    No schema assumptions; best-effort date detection.
# ---------------------------------------------------------------------
$incidentsContent = @'
"use client";
...
Write-Utf8NoBom $incidentsPage $incidentsContent

Write-Host ""
Write-Host "[DONE] PHASE14: Ops Incident Log created + Control Center shortcut added (read-only, GET-only)."
