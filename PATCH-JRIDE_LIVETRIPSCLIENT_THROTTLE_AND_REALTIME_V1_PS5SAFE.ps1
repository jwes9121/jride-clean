# PATCH-JRIDE_LIVETRIPSCLIENT_THROTTLE_AND_REALTIME_V1_PS5SAFE.ps1
param(
...
$backup = Join-Path $backupDir ("LiveTripsClient.tsx.bak.THROTTLE_AND_REALTIME_V1." + $stamp)
...
function parseTripsFromPageData(j: any): TripRow[] {
  // strict canonical trips source only
  if (!j) return [];
...
export default function LiveTripsClient() {
  ...
  const refreshAll = useCallback(async (source?: string) => {
    try {
      ...
      if (source) {
        setLastAction("Refreshed via " + source);
      }
    } catch (e: any) {
      if (source) {
        setLastAction("Refresh failed via " + source + ": " + (e?.message ?? "unknown"));
      }
    }
  ...
  useEffect(() => {
    ...
    const triggerRefresh = (source: string) => {
      ...
      realtimeDebounceRef.current = setTimeout(() => {
        refreshAll(source).catch(() => {});
      }, REALTIME_DEBOUNCE_MS);
    };

    const channelDrivers = supabase
      .channel("livetrips-driver_locations")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "driver_locations" },
        () => triggerRefresh("realtime:driver_locations")
      )
      .subscribe();

    const channelBookings = supabase
      .channel("livetrips-bookings")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "bookings" },
        () => triggerRefresh("realtime:bookings")
      )
      .subscribe();

    channelsRef.current = [channelDrivers, channelBookings];
    setLastAction("Realtime subscribed");
