...
<#
REPLACE JRIDE WEB: verification upload route to use same Supabase server helper as request route
PS5-safe, ASCII-only
...
- Removes manual cookies()/createServerClient auth path
...
Write-Host "== REPLACE JRIDE WEB: verify upload route use createClient (V2 / PS5-safe) ==" -ForegroundColor Cyan
...
$bak = BackupFile $target $bakDir "VERIFY_UPLOAD_USE_CREATECLIENT_V2"
