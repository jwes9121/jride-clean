...
function normalizeMenuRow(row: any) {
  ...
  return {
    ...
    daily_available_quantity: Number.isFinite(Number(row?.daily_available_quantity)) ? Number(row?.daily_available_quantity) : 0,
    remaining_quantity: Number.isFinite(Number(row?.remaining_quantity)) ? Number(row?.remaining_quantity) : 0,
    last_quantity_reset_at: row?.last_quantity_reset_at || null,
    is_active: active,
    is_available: active,
  ...
  try {
    ...
    if (action === "save_item") {
      ...
      const base: Json = {
        ...
        daily_available_quantity: Math.max(0, parseInt(String(body?.daily_available_quantity ?? body?.dailyAvailableQuantity ?? 0), 10) || 0),
        remaining_quantity: Math.max(0, parseInt(String(body?.remaining_quantity ?? body?.remainingQuantity ?? body?.daily_available_quantity ?? body?.dailyAvailableQuantity ?? 0), 10) || 0),
        last_quantity_reset_at: new Date().toISOString(),
        is_active: active,
