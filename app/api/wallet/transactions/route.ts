...
export async function GET(req: Request) {
  try {
    ...
    if (q) {
      if (q.length < 2) {
        return withNoStore(NextResponse.json({ ok: true, drivers: [] }));
      }
      ...
      if (error) {
        return withNoStore(
          NextResponse.json(
            { ok: false, error: "SUGGEST_FAILED", message: error.message },
            { status: 500 }
          )
        );
      }
    ...
    if (!driverId) {
      return withNoStore(
        NextResponse.json(
          { ok: false, error: "MISSING_DRIVER_ID_OR_Q" },
          { status: 400 }
        )
      );
    }
    ...
    if (derr) {
      return withNoStore(
        NextResponse.json(
          { ok: false, error: "DRIVER_READ_FAILED", message: derr.message },
          { status: 500 }
        )
      );
    }
    ...
    if (txErr) {
      return withNoStore(
        NextResponse.json(
          { ok: false, error: "TX_READ_FAILED", message: txErr.message },
          { status: 500 }
        )
      );
    }
