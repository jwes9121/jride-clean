...
function isCompleted(row: any): boolean {
  ...
  return [normStatus(row?.status), normStatus(row?.vendor_status), normStatus(row?.customer_status)].includes("completed");
}

function acceptedEvidence(row: any): boolean {
  if (clean(row?.vendor_accepted_at)) return true;
  if (clean(row?.assigned_at) || clean(row?.takeout_fee_proposed_at) || clean(row?.takeout_customer_confirmed_at)) return true;
  if (clean(row?.vendor_driver_arrived_at) || clean(row?.vendor_order_picked_at) || clean(row?.completed_at)) return true;
...
function decision(row: any): "accepted" | "unaccepted" | "pending" {
  ...
  if (isTimeout(row) || clean(row?.vendor_rejected_at) || status === "cancelled") return "unaccepted";
  return "pending";
}
...
function stats(rows: any[]) {
  ...
  const responses = rows.map(responseSeconds).filter((v): v is number => v !== null && v >= 0);
  ...
  return {
    ...
    acceptance_rate: decided ? Math.round((accepted.length / decided) * 1000) / 10 : null,
    average_response_seconds: responses.length ? Math.round(responses.reduce((a, b) => a + b, 0) / responses.length) : null,
  };
}
...
function groupLabel(status: string): string {
  ...
  if (["pilot", "pilot_lagawe", "active"].includes(status)) return "Pilot / Active";
  return "Unclassified";
}
...
export async function GET(req: NextRequest) {
  ...
  const cohort = clean(req.nextUrl.searchParams.get("cohort") || "pilot_active").toLowerCase();
  const town = clean(req.nextUrl.searchParams.get("town"));
  const now = Date.now();
  const since = new Date(now - days * DAY_MS).toISOString();

  const [vendorsRes, onboardingRes, testAccountsRes, exclusionsRes] = await Promise.all([
    admin.from("vendor_accounts").select("id,email,display_name,town,accepting_orders,created_at,performance_metrics_started_at").order("display_name", { ascending: true }),
    admin.from("vendor_onboarding_credentials").select("vendor_id,status"),
    admin.from("analytics_test_accounts").select("subject_type,subject_id,reason").eq("active", true),
    admin.from("analytics_booking_exclusions").select("booking_id").eq("active", true),
  ]);

  for (const result of [vendorsRes, onboardingRes, testAccountsRes, exclusionsRes]) {
    if (result.error) return json(500, { ok: false, error: "VENDOR_BEHAVIOR_READ_FAILED", message: result.error.message });
  }

  const statusByVendor = new Map<string, string>();
  for (const row of Array.isArray(onboardingRes.data) ? onboardingRes.data : []) {
    statusByVendor.set(clean(row?.vendor_id), clean(row?.status).toLowerCase());
  }
  ...
  const allVendors = (Array.isArray(vendorsRes.data) ? vendorsRes.data : []).filter((vendor: any) => !testVendorIds.has(clean(vendor?.id)));
  ...
  const [bookingsRes, ratingsRes, presenceRes] = await Promise.all([
    ...
    admin
      ...
      .select("booking_id,passenger_id,vendor_id,vendor_rating,created_at")
      .in("vendor_id", vendorIds)
      .gte("created_at", since)
      .limit(10000),
    admin
  ...
  for (const result of [bookingsRes, ratingsRes, presenceRes]) {
    if (result.error) return json(500, { ok: false, error: "VENDOR_BEHAVIOR_DETAIL_READ_FAILED", message: result.error.message });
  }

  const bookings = (Array.isArray(bookingsRes.data) ? bookingsRes.data : []).filter((row: any) => {
    if (testPassengerIds.has(clean(row?.created_by_user_id))) return false;
    if (excludedBookingIds.has(clean(row?.id))) return false;
    return true;
  });
  const bookingById = new Map(bookings.map((r: any) => [clean(r?.id), r] as const));
  ...
  const rows = filteredVendors.map((vendor: any) => {
    ...
    const vendorBookings = bookings.filter((r: any) => clean(r?.vendor_id) === vendorId);
    const vendorStats = stats(vendorBookings);
    const vendorPresence = presence.filter((r: any) => clean(r?.vendor_id) === vendorId);
    const latestPresence = vendorPresence[0] || null;
    const lastSeen = ts(latestPresence?.last_seen_at);
    const currentState = vendor?.accepting_orders !== true ? "closed" : lastSeen >= now - ONLINE_FRESH_MS ? "online" : "open_but_offline";

    const vendorRatings = ratings
      .filter((r: any) => clean(r?.vendor_id) === vendorId)
      .filter((r: any) => !testPassengerIds.has(clean(r?.passenger_id)))
      .filter((r: any) => {
    ...
    const missed = vendorBookings
      ...
      .map((r: any) => ({
        id: clean(r?.id),
        booking_code: clean(r?.booking_code) || clean(r?.id),
        passenger_name: clean(r?.passenger_name) || "Customer",
        amount: Number(r?.takeout_items_subtotal || 0),
        outcome: isTimeout(r) ? "Vendor timeout" : "Vendor rejected",
        reason: clean(r?.vendor_cancel_reason || r?.cancel_reason) || "No reason recorded",
        order_placed_at: r?.created_at || null,
        missed_at: r?.vendor_timeout_at || r?.vendor_rejected_at || r?.vendor_responded_at || r?.updated_at || null,
      }));
    ...
    return {
      ...
      heartbeat_tracking_started_at: vendor?.performance_metrics_started_at || null,
      online_hours_tracked: Math.round((vendorPresence.length / 60) * 10) / 10,
      ...vendorStats,
      survey_responses: vendorRatings.length,
      survey_average: vendorRatings.length ? Math.round((vendorRatings.reduce((a: number, b: number) => a + b, 0) / vendorRatings.length) * 10) / 10 : null,
      average_daily_completed: Math.round((vendorStats.completed / days) * 100) / 100,
      average_daily_unaccepted: Math.round((vendorStats.unaccepted / days) * 100) / 100,
      average_weekly_completed: Math.round((vendorStats.completed / Math.max(1, days / 7)) * 100) / 100,
      average_weekly_unaccepted: Math.round((vendorStats.unaccepted / Math.max(1, days / 7)) * 100) / 100,
      average_monthly_completed: Math.round((vendorStats.completed / Math.max(1, days / 30)) * 100) / 100,
      average_monthly_unaccepted: Math.round((vendorStats.unaccepted / Math.max(1, days / 30)) * 100) / 100,
      recent_missed_orders: missed,
    };
  });

  const cohortCounts: Record<string, number> = { pilot_active: 0, batch2: 0, removed: 0, unclassified: 0 };
  for (const vendor of allVendors) {
    const status = statusByVendor.get(clean(vendor?.id)) || "";
    if (["pilot", "pilot_lagawe", "active"].includes(status)) cohortCounts.pilot_active += 1;
    else if (status === "batch2") cohortCounts.batch2 += 1;
    else if (status === "removed_from_pilot") cohortCounts.removed += 1;
    else cohortCounts.unclassified += 1;
  }
  ...
  return json(200, {
    ...
    order_history_note: "Admin order statistics use real historical Takeout activity inside the selected reporting range. The public vendor-profile baseline remains separate.",
    presence_note: "Online-hour tracking begins only when the new portal heartbeat feature was deployed; historical online hours before that point cannot be reconstructed.",
