...
export async function GET(req: Request) {
  try {
    ...
    const vendor_id = String(searchParams.get("vendor_id") || "");
    ...
    const balUrl = `${SUPABASE_URL}/rest/v1/vendor_wallet_balances_v1?vendor_id=eq.${vendor_id}`;
    const balRes = await fetch(balUrl, { headers: { apikey: SERVICE_KEY, Authorization: `Bearer ${SERVICE_KEY}` } });
    const bal = await balRes.json();

    const txUrl = `${SUPABASE_URL}/rest/v1/vendor_wallet_transactions?vendor_id=eq.${vendor_id}&order=created_at.desc&limit=20`;
