...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error(
        "[JRIDE_DUTY_CHECK_ALERT_RPC_FAILED]",
        error.message
      );

      return json(500, {
        ok: false,
        error: "DUTY_CHECK_ALERT_FAILED",
    ...
    console.error(
      "[JRIDE_DUTY_CHECK_ALERT_UNEXPECTED]",
      error
    );
    ...
    return json(500, {
      ok: false,
      error: "DUTY_CHECK_ALERT_FAILED",
      message: error?.message ?? "Unable to record Duty Check alert.",
    });
  }
}
