...
function deriveTownFromLatLng(lat: number | null, lng: number | null): string | null {
  ...
  const BOXES: Array<{ name: string; minLat: number; maxLat: number; minLng: number; maxLng: number }> = [
    ...
    { name: "Lamut",   minLat: 16.60, maxLat: 16.72, minLng: 121.17, maxLng: 121.28 },
...
export async function GET(req: NextRequest) {
  ...
  const orders = rows.map((r) => {
    ...
    return {
      ...
      takeout_customer_confirmed_at: r?.takeout_customer_confirmed_at ?? null,
      customer_confirmed_at: r?.customer_confirmed_at ?? null,
