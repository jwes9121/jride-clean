...
export async function GET(req: NextRequest) {
  ...
  const vendorRes = await admin
    ...
    .select(
      "id,display_name,email,town,accepting_orders,performance_metrics_started_at"
    )
    .eq("id", vendorId)
    .limit(1)
    .maybeSingle();
  ...
  if (!vendorRes.data) {
    return json(404, { ok: false, error: "VENDOR_NOT_FOUND" });
  }

  const nowMs = Date.now();
  const cutoffMs =
    timestamp(vendorRes.data.performance_metrics_started_at) || nowMs;
  const rangeStartMs = Math.max(cutoffMs, nowMs - days * DAY_MS);
  const rangeStartIso = new Date(rangeStartMs).toISOString();

  const [bookingsRes, testAccountsRes, bookingExclusionsRes, ratingsRes, presenceRes] =
    await Promise.all([
      admin
        .from("bookings")
        .select(
      ...
      admin
        ...
        .select(
          "minute_started_at,last_seen_at,accepting_orders,client"
        )
        .eq("vendor_id", vendorId)
  ...
  for (const result of [
    ...
    if (result.error) {
      return json(500, {
        ok: false,
        error: "VENDOR_SUMMARY_READ_FAILED",
  ...
  const excludedBookingIds = new Set(
    (Array.isArray(bookingExclusionsRes.data)
  ...
  const bookingById = new Map(
    bookings.map((row: any) => [clean(row?.id), row] as const)
  );
  ...
  return json(200, {
    ...
    vendor: {
      vendor_id: vendorId,
      display_name:
        clean(vendorRes.data.display_name) ||
        clean(vendorRes.data.email) ||
        vendorId,
