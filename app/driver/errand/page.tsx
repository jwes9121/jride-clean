          ...
          <>
            ...
            {stage === "awaiting_customer_confirmation" || status === "fare_proposed" ? (
              <section className="rounded-3xl border border-amber-700 bg-amber-950/30 p-5">
                ...
                <div className="mt-4 space-y-2 text-sm"><Fare label="Base" value={fare?.base_fare} /><Fare label={`Confirmed route (${km(job?.confirmed_route_distance_km)})`} value={fare?.distance_fare} />{(num(fare?.night_rate_adjustment) ?? 0) > 0 ? <Fare label={text(fare?.night_rate_mode).toLowerCase() === "double" ? "Night rate (Base + route doubled)" : "Night rate (PHP 100 on Base + route)"} value={fare?.night_rate_adjustment} /> : null}<Fare label="Pickup surcharge" value={fare?.pickup_distance_fee} /><Fare label="Additional stops" value={fare?.extra_stop_fee} /><Fare label="Heavy load" value={fare?.heavy_load_fee} /><div className="border-t border-slate-700 pt-2"><Fare label="Starting service fare" value={fare?.total_errand_fare} /></div></div>
                {(num(fare?.night_rate_adjustment) ?? 0) > 0 ? <div className="mt-2 text-xs text-amber-200">Night rate applies only to Base + route. Pickup and other Errand adjustments are unchanged.</div> : null}
