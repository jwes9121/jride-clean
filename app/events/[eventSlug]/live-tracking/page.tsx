...
function markerColor(freshness: PositionRow["freshness"]) {
...
function popupNode(position: PositionRow) {
  ...
  status.textContent = `Location ${relativeAge(position.ageSeconds)} | ${
    position.accuracyM == null
      ? "accuracy unknown"
      : `about ${Math.round(position.accuracyM)} m accuracy`
  }`;
...
export default function EventLiveTrackingPage() {
  ...
  const [data, setData] = React.useState<LiveTrackingResponse | null>(null);
  ...
  async function load(background = false) {
    ...
    try {
      const response = await fetch(
        `/api/events/${encodeURIComponent(eventSlug)}/live-location/admin`,
        {
          cache: "no-store",
        }
      );

      const payload = (await response.json()) as LiveTrackingResponse;

      if (!response.ok || !payload.success) {
        throw new Error(payload.error || "Unable to load live safety tracking.");
      }
  ...
  React.useEffect(() => {
    ...
    if (!mapboxgl.accessToken) {
      setError("NEXT_PUBLIC_MAPBOX_TOKEN is missing. Live positions are listed below but the map cannot load.");
      return;
    }
    ...
    const activeIds = new Set(positions.map((position) => position.attendeeId));

    for (const [attendeeId, marker] of markersRef.current.entries()) {
    ...
    for (const position of positions) {
      let marker = markersRef.current.get(position.attendeeId);
      ...
      if (!marker) {
        ...
        markersRef.current.set(position.attendeeId, newMarker);
        marker = newMarker;
      }
      ...
      element.style.backgroundColor = markerColor(position.freshness);
      element.title = `${position.fullName} - ${relativeAge(position.ageSeconds)}`;
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-[1600px] p-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <Metric label="Checked In" value={summary.checkedIn} />
          <Metric label="Sharing" value={summary.sharing} />
          <Metric label="Fresh <=45s" value={summary.fresh} />
          <Metric label="Delayed 46-120s" value={summary.delayed} />
          <Metric label="Stale >120s" value={summary.stale} />
        </div>
        ...
        {!data?.trackingOpen ? (
          <div className="mt-4 rounded-2xl border border-amber-700 bg-amber-950/40 p-5 text-amber-100">
            <p className="font-black">Tracking is not open yet.</p>
            <p className="mt-2 text-sm">
              {data?.message || "Set the event status to LIVE when the Fun Walk starts."}
            </p>
          </div>
        ) : null}
          ...
          <div className="relative min-h-[620px] overflow-hidden rounded-3xl border border-slate-800 bg-slate-900">
            ...
            {positions.length === 0 ? (
              <div className="absolute inset-0 flex items-center justify-center p-8 text-center">
                <div>
                  <p className="text-2xl font-black">Waiting for participant locations</p>
              ...
              {positions.map((position) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-black">{position.fullName}</p>
                  ...
                  <p className="mt-3 text-xs font-semibold text-slate-300">
                    Updated {relativeAge(position.ageSeconds)}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {position.accuracyM == null
                      ? "GPS accuracy unknown"
                      : `GPS accuracy about ${Math.round(position.accuracyM)} m`}
                  </p>
                </button>
              ))}
            </div>
