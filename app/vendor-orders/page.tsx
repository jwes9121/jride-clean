    ...
    <main ref={workspace} className="vendor-workspace vendor-orders-workspace">
      <div ref={topPanel} className="vendor-orders-top">
        ...
        <div className="vendor-orders-top-content">
          ...
          <p className={"vendor-orders-freshness" + (feed.stale ? " is-stale" : "")} aria-live="off">{freshness}</p>
          {feed.stale && vendorId ? <p className="vendor-focus-connection" role="status">{feed.authRequired ? "Session ended. Sign in again below." : feed.error ? "Connection lost. Order actions are paused until updates resume." : "Checking the latest status before enabling order actions."}</p> : null}
