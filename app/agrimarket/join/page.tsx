...
export default function AgrimarketFarmerJoinPage() {
  ...
  useEffect(() => {
    ...
    if (!markerRef.current) markerRef.current = new mapboxgl.Marker({ draggable: true });
    ...
    markerRef.current.on("dragend", () => {
      ...
      if (point) setPin({ lat: point.lat, lng: point.lng });
    });
  }, [pin]);
  ...
  async function submitApplication(event: React.FormEvent) {
    ...
    setSubmitted(null);
    if (!pin) {
      setError("Place the exact private farm/home pickup pin first.");
      return;
    }
    setSubmitting(true);
  ...
  async function lookupStatus(event: React.FormEvent) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok || payload?.ok === false) {
      setLookupError(payload?.message || payload?.error || "Application not found.");
    } else {
      setLookupResult(payload.application);
    }
  }
    ...
    return (
      <main className="min-h-screen bg-emerald-50 px-4 py-10 text-slate-900">
        <div className="mx-auto max-w-2xl rounded-3xl border border-emerald-200 bg-white p-8 shadow-sm">
          ...
          <p className="mt-3 text-slate-600">JRide can enable farmer onboarding separately from the customer marketplace when recruitment begins.</p>
      ...
      <div className="mx-auto max-w-5xl">
        <div className="rounded-3xl border bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl bg-emerald-50 p-4"><p className="text-xs uppercase text-emerald-700">Joining fee</p><strong className="text-xl text-emerald-900">FREE</strong></div>
            <div className="rounded-2xl bg-emerald-50 p-4"><p className="text-xs uppercase text-emerald-700">Listing fee</p><strong className="text-xl text-emerald-900">FREE</strong></div>
            <div className="rounded-2xl bg-emerald-50 p-4"><p className="text-xs uppercase text-emerald-700">Farmer deduction</p><strong className="text-xl text-emerald-900">0%</strong></div>
          </div>
        </div>
        ...
        <form onSubmit={submitApplication} className="mt-5 grid gap-5 lg:grid-cols-[1fr_1.1fr]">
          <section className="rounded-3xl border bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="text-sm font-semibold">ID type (optional)<input value={form.identity_type} onChange={(e) => setForm({ ...form, identity_type: e.target.value })} className="mt-2 w-full rounded-xl border px-3 py-3" placeholder="Driver's license, National ID, etc." /></label>
            ...
            <p className="mt-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-900">Do not enter your full government ID number. JRide only needs a short reference here; actual verification can be completed during review.</p>
          ...
          <section className="rounded-3xl border bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">Pin the exact farm/home pickup point. Customers will never see this exact location. It is revealed only to the assigned driver after the driver accepts the Agrimarket job.</p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button type="button" onClick={useCurrentLocation} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Use current location</button>
              <span className="text-xs text-slate-500">or tap/drag the pin on the map</span>
            </div>
            <div ref={mapContainerRef} className="mt-3 h-80 overflow-hidden rounded-2xl border bg-slate-100" />
            <div className="mt-3 rounded-xl bg-slate-50 p-3 text-sm">
              {pin ? <>Private pin: <strong>{pin.lat.toFixed(5)}, {pin.lng.toFixed(5)}</strong></> : "No pickup pin selected yet."}
            </div>
            ...
            {submitted ? (
              <div className="mt-4 rounded-2xl bg-emerald-50 p-4 text-emerald-950">
                ...
                <p className="mt-1 text-sm">Save this code: <strong>{submitted.application_code}</strong></p>
                <p className="mt-1 text-sm">{submitted.status_message}</p>
              </div>
            ) : null}
        ...
        <section className="mt-5 rounded-3xl border bg-white p-5 shadow-sm">
          ...
          {lookupResult ? (
            <div className="mt-3 rounded-xl bg-slate-50 p-4">
              <p className="font-bold">{titleCase(lookupResult.status)}</p>
              <p className="mt-1 text-sm text-slate-600">{lookupResult.status_message}</p>
            </div>
          ) : null}
        </section>
      </div>
