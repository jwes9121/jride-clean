    ...
    <FarmerWorkspace section="products" accountCode={sessionCode} onSignOut={() => void signOut()} onRefresh={() => void loadProducts()} loading={loading || restoring || Boolean(busy)}>
        ...
        {showCreate && <section ref={createPanel} id="new-product" className={styles.createPanel}>
          ...
          <fieldset className={styles.formSection}><legend><span>02</span> Pickup & delivery</legend><div className={styles.formGrid}>
            ...
            <label className="text-sm font-semibold">Vehicle<select value={form.vehicle_requirement} onChange={(e) => setForm({ ...form, vehicle_requirement: e.target.value })} className="mt-1 w-full rounded-xl border bg-white px-3 py-3"><option value="either">Motorcycle / Tricycle / Kolong-Kolong</option><option value="motorcycle">Motorcycle</option><option value="tricycle">Tricycle</option><option value="kolong_kolong">Kolong-Kolong</option></select></label>
          ...
          {visibleProducts.map((product) => {
            ...
            return <article key={product.id} className={styles.productCard}>
              ...
              <details className={styles.editDetails}>
                <summary>Edit transport weight<span className="sr-only"> for {product.name}</span></summary>
