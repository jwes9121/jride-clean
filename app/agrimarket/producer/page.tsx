...
import { weightConfirmationError } from "@/lib/agrimarket/weightValidation";
...
import { estimatedHarvestEnd, manilaDateTimeToIso, scheduledActivity, scheduledTitle } from "@/lib/agrimarket/schedule";
      ...
      <div className={!orders.length && !viewingHistory ? styles.dashboardGrid : undefined}>
        <section className={styles.orderList} aria-label={viewingHistory ? "Order history" : "Farmer orders"} aria-busy={viewingHistory ? historyLoading : loading}>
          ...
          {visibleOrders.map((order) => {
            ...
            return (
              <article data-agri-review-order={order.order_code} id={`agri-order-${order.order_code}`} key={order.order_code} className={styles.orderCard}>
                ...
                {scheduled ? <div className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-900"><strong>{scheduledTitle(order.items)} window</strong><br/>{formatDate(order.harvest_expected_start_at)}{estimatedHarvestEnd(order.harvest_expected_start_at, order.harvest_expected_end_at) ? ` to ${formatDate(estimatedHarvestEnd(order.harvest_expected_start_at, order.harvest_expected_end_at))}` : ""}{!order.harvest_expected_end_at ? " (estimated one-hour window)" : ""}</div> : null}
                ...
                {order.status === "awaiting_producer" ? (
                  <div className="mt-4 rounded-2xl bg-blue-50 p-4">
                    ...
                    {!scheduled && weightError && <p id={weightHelpId} role="status" className="mt-3 rounded-lg border border-amber-400 bg-amber-50 p-3 text-sm font-semibold text-amber-950">Accept is unavailable: {weightError}</p>}
                    <div className="mt-3 flex gap-2"><button aria-describedby={!scheduled && weightError ? weightHelpId : undefined} disabled={busy.includes(order.order_code) || (!scheduled && Boolean(weightError))} onClick={() => decide(order, "accept")} className="rounded-xl bg-emerald-700 px-4 py-2 font-bold text-white disabled:bg-slate-400">{busy.includes(order.order_code) ? "Saving..." : scheduled ? "Accept reservation" : "Accept"}</button><button disabled={busy.includes(order.order_code)} onClick={() => decide(order, "reject")} className="rounded-xl bg-red-700 px-4 py-2 font-bold text-white">Cannot fulfill</button></div>
                  </div>
                ) : null}
                ...
                {scheduled && activity === "butchering" && ["awaiting_producer", "awaiting_harvest"].includes(order.status) && <CancelReservation
                  ...
                  onBusy={value => setBusy(value ? `cancel-${order.order_code}` : "")}
                  onComplete={async notice => { setMessage(notice); await loadOrders(sessionCode, true); setHistoryRefresh(value => value + 1); }}
                />}
