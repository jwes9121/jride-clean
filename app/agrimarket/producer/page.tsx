...
type ProducerOrder = {
  ...
  producer_marketplace_commission: number;
  producer_product_net: number;
  producer_paid_at?: string | null;
  producer_paid_amount: number;
  producer_payment_status: string;
...
function orderStep(order: ProducerOrder): string {
  ...
  if (order.picked_up_at || order.status === "picked_up" || order.status === "delivering") return "Picked up by driver";
...
export default function AgrimarketProducerPage() {
  ...
  useEffect(() => {
    ...
    setVendorId(window.sessionStorage.getItem(SESSION_VENDOR_ID) || "");
    setPin(window.sessionStorage.getItem(SESSION_VENDOR_PIN) || "");
  }, []);

  async function loadOrders(id = vendorId, accessPin = pin, quiet = false) {
    if (!id.trim() || !accessPin.trim()) return;
    if (!quiet) setLoading(true);
    setError("");

    const { response, payload } = await apiJson("/api/agrimarket/producer/orders", {
    ...
    if (payload?.error === "AGRIMARKET_DISABLED") {
      ...
      setConnected(false);
      setError(payload?.message || "The farmer credentials were not accepted.");
      ...
      setPrepByOrder((current) => {
        ...
        for (const order of rows as ProducerOrder[]) {
          if (next[order.order_code] == null) {
            const preferred = PREP_OPTIONS.includes(Number(order.preparation_minutes))
              ? Number(order.preparation_minutes)
              : 15;
            next[order.order_code] = preferred;
          }
        }
        return next;
      });
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void loadOrders(vendorId, pin, true);
    }, 15000);
    return () => window.clearInterval(timer);
  }, [connected, disabled, vendorId, pin]);

  async function decide(orderCode: string, decision: "accept" | "reject") {
    setBusyOrder(orderCode);
    setError("");
    setMessage("");
    const { response, payload } = await apiJson("/api/agrimarket/producer/orders/decision", {
      method: "POST",
    ...
    if (!response.ok || payload?.ok === false) {
      ...
      await loadOrders(vendorId, pin, true);
    }
    setBusyOrder("");
  }
  ...
  function signOut() {
    ...
    setError("");
    if (typeof window !== "undefined") {
      window.sessionStorage.removeItem(SESSION_VENDOR_ID);
      window.sessionStorage.removeItem(SESSION_VENDOR_PIN);
    }
  }
    ...
    return (
      <main className="min-h-screen bg-emerald-50 px-4 py-10 text-slate-900">
        <div className="mx-auto max-w-2xl rounded-3xl border border-emerald-200 bg-white p-8 shadow-sm">
          ...
          <p className="mt-4 text-slate-600">Agrimarket remains disabled while the farmer and delivery workflow is being completed and tested.</p>
          <Link href="/vendor" className="mt-6 inline-flex rounded-xl bg-emerald-700 px-5 py-3 font-semibold text-white">Back to vendor portal</Link>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-10 text-slate-900">
        <div className="mx-auto max-w-md rounded-3xl border bg-white p-7 shadow-sm">
          ...
          <h1 className="mt-2 text-2xl font-bold">Farmer order console</h1>
          <p className="mt-2 text-sm text-slate-600">Use your JRide vendor credentials. Your PIN is kept only for this browser session.</p>
          {error ? <div className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-800">{error}</div> : null}
          <label className="mt-5 block text-sm font-semibold">
            Vendor ID
            <input value={vendorId} onChange={(event) => setVendorId(event.target.value)} className="mt-2 w-full rounded-xl border px-3 py-3" placeholder="Vendor UUID" />
          </label>
          <label className="mt-4 block text-sm font-semibold">
            Access PIN
            <input type="password" value={pin} onChange={(event) => setPin(event.target.value)} className="mt-2 w-full rounded-xl border px-3 py-3" placeholder="PIN" />
          </label>
          <button type="button" onClick={() => loadOrders()} disabled={loading || !vendorId.trim() || !pin.trim()} className="mt-5 w-full rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white disabled:bg-slate-400">
            {loading ? "Checking..." : "Open farmer console"}
          </button>
          <Link href="/vendor" className="mt-4 block text-center text-sm font-semibold text-slate-600">Back to vendor portal</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-slate-50 px-3 py-5 text-slate-900 sm:px-5">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-5 space-y-4">
          {orders.length === 0 ? (
            <div className="rounded-2xl border bg-white p-8 text-center text-slate-500">No active Agrimarket orders.</div>
          ) : orders.map((order) => {
              ...
              <article key={order.order_code} className="rounded-2xl border bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-1 text-xl font-bold">{orderStep(order)}</h2>
                  ...
                  {awaiting ? (
                    <div className="rounded-xl bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-900">
                      Confirm within about {Math.ceil(order.confirmation_seconds_remaining / 60)} min
                    </div>
                  ) : null}
                </div>
                    ...
                    <div key={`${order.order_code}-${index}`} className="flex flex-wrap justify-between gap-2 border-b px-4 py-3 last:border-b-0">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {item.quantity} {item.selling_unit} x {money(item.unit_price)}
                          {item.condition_required ? ` - ${titleCase(item.condition_required)}` : ""}
                        </p>
                      </div>
                ...
                {order.producer_paid_at ? (
                  <div className="mt-3 rounded-xl bg-blue-50 p-3 text-sm text-blue-900">
                    Driver payment recorded: {money(order.producer_paid_amount)}. Pickup verification follows before the goods leave the farm.
                  </div>
                ) : null}
                ...
                {awaiting ? (
                  <div className="mt-4 rounded-xl border p-4">
                    <label className="text-sm font-semibold">
                      Preparation time after acceptance
                      <select
                        value={prepByOrder[order.order_code] ?? 15}
                        onChange={(event) => setPrepByOrder((current) => ({ ...current, [order.order_code]: Number(event.target.value) }))}
                        className="mt-2 w-full rounded-xl border bg-white px-3 py-3 sm:max-w-xs"
                      >
                        {PREP_OPTIONS.map((minutes) => <option key={minutes} value={minutes}>{minutes === 0 ? "Ready now" : `${minutes} minutes`}</option>)}
                      </select>
                    </label>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button type="button" disabled={busyOrder === order.order_code} onClick={() => decide(order.order_code, "accept")} className="rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white disabled:bg-slate-400">Accept order</button>
                      <button type="button" disabled={busyOrder === order.order_code} onClick={() => decide(order.order_code, "reject")} className="rounded-xl border border-red-300 px-5 py-3 font-semibold text-red-700 disabled:text-slate-400">Cannot fulfill</button>
                    </div>
                  </div>
                ) : (
                  <div className="mt-4 text-sm text-slate-600">
                    <p>Preparation: {order.preparation_minutes} min</p>
                    {order.ready_at ? <p>Target ready time: {new Date(order.ready_at).toLocaleString()}</p> : null}
                    <p>Requested vehicle: {titleCase(order.preferred_vehicle_type)}</p>
                  </div>
                )}
              </article>
            );
