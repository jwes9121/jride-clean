...
function isStaleDriver(d: any): boolean {

  const v = d?.location_updated_at ?? d?.updated_at ?? d?.last_seen ?? null;
...
function fleetMarkerColor(d: any): { bg: string; ring: string; show: boolean } {
  ...
  if (isStaleDriver(d)) {
    return { bg: "#999999", ring: "#999999", show: false };
  }
...
const pending = trips.filter((t: any) =>
  ...
  useEffect(() => {
    ...
    for (const d of list) {
      ...
      const color = fleetMarkerColor(d);
