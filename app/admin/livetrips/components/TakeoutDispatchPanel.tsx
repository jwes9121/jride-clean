...
export default function TakeoutDispatchPanel() {
    ...
    <section className="min-h-screen bg-slate-50 p-4 text-slate-900">
      <div className="mx-auto max-w-7xl space-y-4">
        <section className="rounded-2xl border bg-white p-4 shadow-sm">
          ...
          <div className="mt-4">
            <label htmlFor="takeout-dispatch-search" className="mb-1 block text-sm font-semibold text-slate-700">
              Search orders
            </label>
                    ...
                    <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                      <div className="min-w-0 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={["rounded-full border px-2 py-1 text-xs font-semibold", statusClass(status, o.is_stuck)].join(" ")}>
                            {o.is_stuck
                              ? `${o.proposal_expiry_stuck ? "STUCK - Proposal Expired - " : "STUCK - "}${titleCase(status)}`
                              : titleCase(status)}
                          </span>
