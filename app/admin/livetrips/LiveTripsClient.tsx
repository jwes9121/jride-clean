...
export default function LiveTripsClient() {
  ...
  function getTripTime(t: TripRow): number {
    const raw = t.completed_at || t.cancelled_at || t.updated_at || t.created_at || null;
    const n = raw ? new Date(raw).getTime() : 0;
    return Number.isFinite(n) ? n : 0;
  }

  function isInsideDateRange(t: TripRow): boolean {
    const time = getTripTime(t);
  ...
  function matchesTown(t: TripRow): boolean {
    if (townFilter === "all") return true;
    const town = String(t.town || t.zone || "").trim().toLowerCase();
    return town === townFilter.toLowerCase();
  }
  ...
  const visibleTrips = useMemo(() => {
    ...
    } else if (f === "completed" || f === "cancelled") {
      out = allTrips.filter((t) => normStatus(t.status) === f && isInsideDateRange(t));
    } else {
      out = allTrips.filter((t) => normStatus(t.status) === f);
    }

    out = out.filter(matchesTown);
    ...
    return out;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [allTrips, tripFilter, stuckTripIds, dateRange, townFilter]);
  ...
  const opsStats = useMemo(() => {
    const scoped = allTrips.filter(matchesTown);
    const historical = scoped.filter((t) => isInsideDateRange(t));
    const completed = historical.filter((t) => normStatus(t.status) === "completed").length;
    const cancelled = historical.filter((t) => normStatus(t.status) === "cancelled").length;
    const active = scoped.filter((t) => isActiveTripStatus(normStatus(t.status))).length;
    return {
  ...
  }, [allTrips, drivers.length, dateRange, townFilter]);

  const townOptions = useMemo(() => {
    const names = new Set<string>();
    for (const z of zones) if (z.zone_name) names.add(String(z.zone_name));
    for (const t of allTrips) {
      const name = String(t.town || t.zone || "").trim();
  ...
  return (
    <div className="p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold">LiveTrips Command Center</h1>
          <p className="text-sm text-gray-600">Premium dispatch workspace for live monitoring, driver readiness, and historical trip review.</p>
        </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button className={pillClass(tripFilter === "dispatch")} onClick={() => setFilterAndFocus("dispatch")}>Active dispatch</button>
        <button className={pillClass(tripFilter === "completed")} onClick={() => setFilterAndFocus("completed")}>Completed</button>
        <button className={pillClass(tripFilter === "cancelled")} onClick={() => setFilterAndFocus("cancelled")}>Cancelled</button>

        <select
          className="rounded-full border border-gray-200 bg-white px-3 py-1 text-sm"
          value={dateRange}
          onChange={(e) => setDateRange(e.target.value as any)}
          disabled={!["completed", "cancelled"].includes(tripFilter)}
        >
        ...
        <button
          ...
          onClick={() => { setTownFilter("all"); setTripFilter("dispatch"); }}
        >
          Clear filters
        </button>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3"><div className="text-xs font-semibold text-blue-700">Trips in range</div><div className="text-xl font-bold">{opsStats.total}</div></div>
        <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-3"><div className="text-xs font-semibold text-emerald-700">Completed</div><div className="text-xl font-bold">{opsStats.completed}</div></div>
        <div className="rounded-lg border border-red-100 bg-red-50 p-3"><div className="text-xs font-semibold text-red-700">Cancelled</div><div className="text-xl font-bold">{opsStats.cancelled}</div></div>
        <div className="rounded-lg border border-amber-100 bg-amber-50 p-3"><div className="text-xs font-semibold text-amber-700">Active now</div><div className="text-xl font-bold">{opsStats.active}</div></div>
        <div className="rounded-lg border border-slate-200 bg-white p-3"><div className="text-xs font-semibold text-slate-700">Drivers loaded</div><div className="text-xl font-bold">{opsStats.drivers}</div></div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        {zones.map((z) => (
          <button key={z.zone_id} type="button" onClick={() => setTownFilter(String(z.zone_name || "all"))} className="rounded-lg border p-3 text-left hover:bg-gray-50">
        ...
        <div className="rounded-lg border">
          <div className="p-3 border-b flex items-center justify-between">
            <div className="font-semibold">
              {tripFilter === "dispatch" ? "Dispatch view (Pending + Assigned + On the way)" : tripFilter === "completed" ? `Completed trips (${dateRange})` : tripFilter === "cancelled" ? `Cancelled trips (${dateRange})` : "Trips"}
            </div>
          ...
          <div className="p-3 border-t">
            ...
            <div className="mt-3 rounded border p-3">
              ...
              <div className="mt-2">
                <SmartAutoAssignSuggestions trip={selectedTrip as any} drivers={drivers as any} zoneStats={{}} onAssign={(driverId) => selectedTrip?.booking_code ? assignDriver(selectedTrip.booking_code, driverId) : undefined} />
              </div>
            </div>
          </div>
        </div>
