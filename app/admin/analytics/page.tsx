...
export default function AdminAnalyticsPage() {
  ...
  const [presenceFreshnessSeconds, setPresenceFreshnessSeconds] =
    React.useState<number>(90);
  const [presenceCounts, setPresenceCounts] = React.useState<
    NonNullable<PresenceResponse["counts"]>
  >({
    active_now: 0,
    foreground_now: 0,
    background_now: 0,
  ...
  const [driverWorkforceRows, setDriverWorkforceRows] = React.useState<
    DriverWorkforceRow[]
  >([]);
  ...
  const [driverWorkforceStatus, setDriverWorkforceStatus] =
    React.useState<string>("Not loaded yet.");
  const load = React.useCallback(async () => {
    setLoading(true);
    setMsg("");
    try {
      const [tripRes, driverRes, watchRes, failRes, presenceRes] =
        await Promise.all([
          fetch("/api/admin/analytics/trips", {
            cache: "no-store",
          ...
          fetch("/api/admin/analytics/driver-watchlist?limit=8", {
            cache: "no-store",
            credentials: "same-origin",
          }),
          fetch("/api/admin/analytics/no-driver-searches?limit=100", {
            cache: "no-store",
            credentials: "same-origin",
          }),
          fetch(
            "/api/admin/analytics/passenger-presence?freshness_seconds=90",
            { cache: "no-store", credentials: "same-origin" },
          ),
        ]);
      const tripsJson = (await tripRes
        .json()
        .catch(() => ({}))) as TripsResponse;
      const driversJson = (await driverRes
        .json()
        .catch(() => ({}))) as DriversResponse;
      const watchJson = (await watchRes
        .json()
        .catch(() => ({}))) as WatchlistResponse;
      const failJson = (await failRes
        .json()
        .catch(() => ({}))) as FailuresResponse;
      const presenceJson = (await presenceRes
        .json()
        .catch(() => ({}))) as PresenceResponse;
      if (!tripRes.ok)
        throw new Error(tripsJson.error || "Failed to load trips analytics.");
      if (!driverRes.ok)
        throw new Error(
          driversJson.error || "Failed to load driver analytics.",
        );
      if (!watchRes.ok)
        throw new Error(watchJson.error || "Failed to load driver watchlist.");
      if (!failRes.ok)
        throw new Error(
          failJson.error || "Failed to load no-driver analytics.",
        );
      if (!presenceRes.ok)
        throw new Error(
      ...
      setPresenceRows(
        Array.isArray(presenceJson.rows) ? presenceJson.rows : [],
      );
      setPresenceTowns(
        Array.isArray(presenceJson.towns) ? presenceJson.towns : [],
      );
      ...
      setPresenceCounts({
        ...
        offline_marked_now: Number(
          presenceJson.counts?.offline_marked_now || 0,
        ),
        with_booking_now: Number(presenceJson.counts?.with_booking_now || 0),
        searching_now: Number(presenceJson.counts?.searching_now || 0),
      });
      try {
        const workforceRes = await fetch(
          "/api/admin/analytics/driver-workforce?freshness_seconds=180",
          { cache: "no-store", credentials: "same-origin" },
        );
        if (workforceRes.ok) {
          const workforceJson = (await workforceRes
            .json()
            .catch(() => ({}))) as DriverWorkforceResponse;
          setDriverWorkforceRows(
            Array.isArray(workforceJson.rows) ? workforceJson.rows : [],
          );
          ...
          setDriverWorkforceSummary({
            ...
            drivers_logged_in_today: Number(
              workforceJson.summary?.drivers_logged_in_today || 0,
            ),
            total_online_minutes_today: Number(
              workforceJson.summary?.total_online_minutes_today || 0,
            ),
            avg_online_minutes_today: Number(
              workforceJson.summary?.avg_online_minutes_today || 0,
            ),
            qualified_today: Number(
              workforceJson.summary?.qualified_today || 0,
            ),
            qualified_this_week: Number(
              workforceJson.summary?.qualified_this_week || 0,
            ),
            no_driver_minutes_today: Number(
              workforceJson.summary?.no_driver_minutes_today || 0,
            ),
            worst_gap_start_at:
              workforceJson.summary?.worst_gap_start_at || null,
            worst_gap_end_at: workforceJson.summary?.worst_gap_end_at || null,
            worst_gap_minutes: Number(
              workforceJson.summary?.worst_gap_minutes || 0,
            ),
          });
          ...
          setDriverCoverageGaps([]);
          setDriverWorkforceStatus(
            "Driver workforce endpoint is not available yet. Existing analytics remain unchanged.",
          );
        }
      } catch {
        setDriverWorkforceRows([]);
  ...
  const scopedFailures = React.useMemo(
    () => failureRows.filter((r) => townMatch(scope, r.town)),
    [failureRows, scope],
  );
  const scopedPresenceRows = React.useMemo(
    () => presenceRows.filter((r) => townMatch(scope, r.town)),
    [presenceRows, scope],
  );
  ...
  const scopedDriverWorkforceRows = React.useMemo(
    () =>
      driverWorkforceRows.filter((r) =>
        townMatch(scope, r.town ?? r.municipality),
      ),
    [driverWorkforceRows, scope],
  );
  const scopedDriverCoverageGaps = React.useMemo(
  ...
  const driverWorkforceTotals = React.useMemo(() => {
    ...
    const activeNow = rows.filter(
      (r) =>
        !!r.is_online_now ||
        String(r.current_status || "").toLowerCase() === "online",
    ).length;
    const loggedInToday = rows.filter(
      (r) =>
        Number(r.today_online_minutes || 0) > 0 ||
        Number(r.today_login_count || 0) > 0,
    ).length;
    const totalToday = rows.reduce(
      (sum, r) => sum + Number(r.today_online_minutes || 0),
      0,
    );
    const avgToday =
      loggedInToday > 0 ? Math.round(totalToday / loggedInToday) : 0;
    const qualifiedToday = rows.filter(
      (r) => Number(r.today_online_minutes || 0) >= 480,
    ).length;
    const qualifiedWeek = rows.filter(
      (r) => Number(r.week_qualified_days || 0) >= 5,
    ).length;
    const noDriverMinutes =
      scope === "all"
        ? Number(driverWorkforceSummary.no_driver_minutes_today || 0)
        : scopedDriverCoverageGaps.reduce(
            (sum, r) => sum + Number(r.minutes || 0),
            0,
          );
    const worstGap =
      scopedDriverCoverageGaps
        .slice()
  ...
  const driverLiveCoverageRows = React.useMemo(() => {
    ...
    for (const row of scopedDriverWorkforceRows) {
      ...
      if (
        !!row.is_online_now ||
        String(row.current_status || "").toLowerCase() === "online"
      ) {
        current.online += 1;
        current.fresh += 1;
      }
      ...
      if (last && (!prev || last > prev))
        current.lastSeen = row.last_seen_at || null;
    ...
    return scopedDriverWorkforceRows
      .map((row) => {
        const lastSeenMs = row.last_seen_at
          ...
          : 0;
        const daysInactive =
          lastSeenMs > 0 ? Math.floor((nowMs - lastSeenMs) / 86400000) : 999;
        ...
        const acceptanceRate =
          assigned > 0 ? Math.round((accepted / assigned) * 100) : null;
        const walletBalance =
          row.wallet_balance == null ? null : Number(row.wallet_balance);
        const hasLowWallet =
          walletBalance !== null &&
        ...
        if (assigned === 0 && completed === 0)
          flags.push("New / never activated");
        if (hasLowWallet) flags.push("Low wallet");
        if (assigned > 0 && accepted === 0) flags.push("No accepts");
        if (acceptanceRate !== null && acceptanceRate < 50)
          flags.push("Low accept");
        if (completed === 0 && assigned > 0) flags.push("No completed trips");

        let priority = 0;
        if (daysInactive >= 14) priority += 40;
        else if (daysInactive >= 7) priority += 25;
        if (hasLowWallet) priority += 20;
        if (assigned === 0 && completed === 0) priority += 15;
        ...
        return {
          ...
          lastTrip: row.last_completed_trip_at || null,
          walletBalance,
          assigned,
          accepted,
          completed,
      ...
      .sort((a, b) => {
        ...
        if (b.daysInactive !== a.daysInactive)
  ...
  const driverActivationSummary = React.useMemo(() => {
    const offline7 = driverActivationRows.filter(
      (row) => row.daysInactive >= 7,
    ).length;
    const offline14 = driverActivationRows.filter(
      (row) => row.daysInactive >= 14,
    ).length;
    const lowAccept = driverActivationRows.filter(
      (row) =>
        row.flags.includes("Low accept") || row.flags.includes("No accepts"),
    ).length;
    const lowWallet = driverActivationRows.filter((row) =>
      row.flags.includes("Low wallet"),
    ).length;
    const neverActivated = driverActivationRows.filter((row) =>
      row.flags.includes("New / never activated"),
    ).length;
    return {
      offline7,
      offline14,
      ...
      lowWallet,
      neverActivated,
      total: driverActivationRows.length,
    };
  }, [driverActivationRows]);
  const presenceTotals = React.useMemo(() => {
    const activeNow = scopedPresenceRows.length;
    const foregroundNow = scopedPresenceRows.filter(
      (r) => String(r.app_state || "foreground") === "foreground",
    ).length;
    const backgroundNow = scopedPresenceRows.filter(
      (r) => String(r.app_state || "") === "background",
    ).length;
    const offlineMarkedNow = scopedPresenceRows.filter(
      (r) => String(r.app_state || "") === "offline",
    ).length;
    const withBookingNow = scopedPresenceRows.filter(
      (r) => !!r.last_booking_code,
    ).length;
    const searchingNow = scopedPresenceRows.filter((r) => {
      ...
      return (
        !r.last_booking_code &&
        [
          "passengerbookrideactivity",
          "passengersearchingactivity",
          "search",
          "booking",
          "home",
        ].includes(screen)
  ...
  const totals = React.useMemo(() => {
    const totalTrips = scopedTrips.reduce(
    ...
    const todaShareTotal = scopedTrips.reduce(
      (sum, row) => sum + Number(row.toda_share_total || 0),
      0,
    );
    const todaTrips = scopedTrips.reduce(
    ...
    const towns = new Set(
      scopedTrips.map((r) => String(r.town || "Unknown (legacy data)")),
    );
    return {
      totalTrips,
      companyShareTotal,
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 md:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-6 p-6 md:p-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-4 text-3xl font-bold tracking-tight md:text-4xl">
                Operations analytics, company share, TODA share, passenger
                presence, driver workforce, and no-driver demand
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 md:text-base">
                Built for expansion. View town-filtered results, export CSV
                reports, and monitor partner-safe metrics using Philippine date
                and time.
              </p>
            </div>
            <div className="grid w-full grid-cols-1 gap-3 text-sm text-slate-300 sm:w-auto sm:min-w-[280px] sm:grid-cols-1 xl:grid-cols-2">
              <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                ...
                <div className="mt-1 break-words font-semibold leading-snug text-white">
                  Asia/Manila (PHT)
                </div>
              </div>
              <div className="min-w-0 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Use town filtering now so future partner access can stay
                town-scoped.
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                onClick={exportDriverWorkforce}
              >
                Export driver workforce CSV
              </button>
            </div>
          </div>
          {msg ? (
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
          ...
          <Card
            ...
            sub="PHP 1 per TODA member ride"
          />
          <Card
            title="TODA rides"
            value={formatCount(totals.todaTrips)}
            sub="Completed trips with a TODA member driver"
          />
          <Card
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
            <Card
              ...
              value={formatCount(
                driverWorkforceTotals.activeNow ||
                  Number(driverWorkforceSummary.active_drivers_now || 0),
              )}
              sub="Fresh online drivers in scope"
            />
            <Card
            ...
            <Card
              ...
              value={formatMinutesLabel(
                driverWorkforceTotals.totalToday ||
                  Number(
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <h3 className="text-sm font-semibold text-slate-900">
                Driver recent activity ranking
              </h3>
              <p className="mt-1 text-xs text-slate-500">
                Uses driver_locations recent activity plus booking acceptance
                data. This panel is display-only and does not change dispatch,
                wallets, or lifecycle.
              </p>
                ...
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 bg-white">
                    <tr className="border-b border-slate-200 text-left text-slate-500">
                      ...
                      <th className="py-2 pr-4 font-semibold">Today</th>
                  ...
                  <tbody>
                    {scopedDriverWorkforceRows.length === 0 ? (
                      ...
                      scopedDriverWorkforceRows.slice(0, 25).map((row, idx) => (
                        <tr
                          key={String(
                            row.driver_id ||
                        ...
                        >
                          ...
                          <td className="py-2 pr-4 text-slate-700">
                            {row.town ?? row.municipality ?? "Unknown"}
                          </td>
                          <td className="py-2 pr-4 text-slate-700">
                            {formatMinutesLabel(row.today_online_minutes)}
                          </td>
                          <td className="py-2 pr-4 text-slate-700">
                            {formatCount(Number(row.today_login_count || 0))}
                          </td>
                          <td className="py-2 pr-4 text-slate-700">
                            {formatCount(Number(row.week_qualified_days || 0))}{" "}
                            / 5
                          </td>
                          <td className="py-2 pr-4 text-slate-700">
                            {formatAcceptanceRate(
                              row.accepted_bookings,
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  ...
                  <div className="mt-1 text-2xl font-bold text-slate-950">
                    {formatCount(scopedDriverWorkforceRows.length)}
                  </div>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
              ...
              <div className="mt-3 max-h-[390px] overflow-auto rounded-xl border border-slate-200 bg-white">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 bg-white">
                    <tr className="border-b border-slate-200 text-left text-slate-500">
                      <th className="py-2 pl-3 pr-4 font-semibold">Town</th>
                      <th className="py-2 pr-4 font-semibold">Online</th>
                      <th className="py-2 pr-4 font-semibold">Roster</th>
                      <th className="py-2 pr-3 font-semibold">Latest seen</th>
                    </tr>
                  </thead>
                  <tbody>
                    {driverLiveCoverageRows.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-3 py-4 text-slate-400">
                          No live driver coverage rows for this scope.
                        </td>
                      </tr>
                    ) : (
                      driverLiveCoverageRows.map((row) => (
                        <tr
                          key={row.town}
                          className="border-b border-slate-100 last:border-0"
                        >
                          ...
                          <td className="py-2 pr-4 text-slate-700">
                            {formatCount(row.online)}
                          </td>
                          <td className="py-2 pr-4 text-slate-700">
                            {formatCount(row.roster)}
                          </td>
                          <td className="py-2 pr-3 text-slate-700">
                            {formatPHDateTime(row.lastSeen)}
                          </td>
                        </tr>
                      ))
                    )}
              ...
              <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                No-driver gap history remains disabled because no driver session
                history table exists in production. This panel uses only
                truthful live snapshot data.
              </div>
            </div>
          </div>
        </section>
        <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h2 className="text-lg font-bold tracking-tight text-slate-900">
                Driver activation center
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Read-only activation risk view using the same driver_locations
                and bookings snapshot. This does not change dispatch, wallets,
                lifecycle, or driver status.
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
              <div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-5">
            <Card
              title="Offline 7d+"
              value={formatCount(driverActivationSummary.offline7)}
              sub="Needs coordinator follow-up"
            />
            <Card
              title="Offline 14d+"
              value={formatCount(driverActivationSummary.offline14)}
              sub="High reactivation risk"
            />
            <Card
              title="Low wallet"
              value={formatCount(driverActivationSummary.lowWallet)}
              sub="Below PHP 250 when known"
            />
            <Card
              title="Never activated"
              value={formatCount(driverActivationSummary.neverActivated)}
              sub="No assigned or completed trips"
            />
            <Card
              title="Mode"
              value="Read-only"
              sub="No dispatch or wallet writes"
            />
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="sticky top-0 bg-slate-50">
                <tr className="border-b border-slate-200 text-left text-slate-500">
                  ...
                  <th className="py-2 pr-4 font-semibold">Last trip</th>
                  <th className="py-2 pr-4 font-semibold">Last seen</th>
                  <th className="py-2 pr-3 font-semibold">Quick actions</th>
                </tr>
              </thead>
              <tbody>
                {driverActivationRows.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-3 py-4 text-slate-400">
                      No driver activation risks found for this scope.
                    </td>
                  </tr>
                ) : (
                  driverActivationRows.map((row) => (
                    <tr
                      key={String(row.driver_id || row.driver_name)}
                      className="border-b border-slate-100 last:border-0"
                    >
                      <td className="py-2 pl-3 pr-4 font-medium text-slate-900">
                        {row.driver_name}
                      ...
                      <td className="py-2 pr-4 text-slate-700">
                        <span
                          className={
                            ...
                            row.walletBalance < 250
                              ? "font-semibold text-amber-700"
                              : ""
                          }
                        >
                          {formatWalletBalance(row.walletBalance)}
                        </span>
                      </td>
                      <td className="py-2 pr-4 text-slate-700">
                      ...
                      <td className="py-2 pr-3 text-slate-700">
                        {cleanPhone(row.phone) ? (
                          <div className="flex flex-wrap gap-1">
                            ...
                            <a
                              className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50"
                              href={`tel:${cleanPhone(row.phone)}`}
                            >
                              Call
                            </a>
                            <a
                              className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50"
                              href={`sms:${cleanPhone(row.phone)}`}
                            >
                              Message
                            </a>
                          </div>
          ...
          <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800">
            Suggested use: call or message these drivers, check wallet
            readiness, review last completed trip, and coordinate with TODA
            leads. This panel is display-only.
          </div>
        </section>
        <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h2 className="text-lg font-bold tracking-tight text-slate-900">
                Passenger presence live panel
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Read-only live demand view from passenger_app_presence. Fresh
                rows only. Current freshness window:{" "}
                {formatSecondsLabel(presenceFreshnessSeconds)}.
              </p>
            </div>
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
              ...
              <div className="mt-1 font-semibold">
                {scope === "all" ? "All towns" : scope}
              </div>
              <div className="mt-1 text-xs text-emerald-700">
                API total active now:{" "}
                {formatCount(Number(presenceCounts.active_now || 0))}
              </div>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
            <Card
              title="Active passengers now"
              value={formatCount(presenceTotals.activeNow)}
              sub="Fresh presence rows in scope"
            />
            <Card
              title="Foreground now"
              value={formatCount(presenceTotals.foregroundNow)}
              sub="App_state = foreground"
            />
            <Card
              title="Searching now"
              value={formatCount(presenceTotals.searchingNow)}
              sub="No booking code on booking/search/home screens"
            />
            <Card
              title="Background now"
              value={formatCount(presenceTotals.backgroundNow)}
              sub="App_state = background"
            />
            <Card
              title="With booking now"
              value={formatCount(presenceTotals.withBookingNow)}
              sub="Presence rows carrying a booking code"
            />
          </div>
          <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Counts are filtered by the selected town scope.
              </p>
              <div className="mt-3 overflow-auto">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-left text-slate-500">
                      <th className="py-2 pr-4 font-semibold">Town</th>
                      <th className="py-2 font-semibold">Active now</th>
                    </tr>
                  </thead>
                  <tbody>
                    {scopedPresenceTowns.length === 0 ? (
                      <tr>
                      ...
                      scopedPresenceTowns.map((row) => (
                        <tr
                          key={String(row.town || "Unknown")}
                          className="border-b border-slate-100 last:border-0"
                        >
                          ...
                          <td className="py-2 text-slate-700">
                            {formatCount(Number(row.active_now || 0))}
                          </td>
                        </tr>
                      ))
                    )}
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <h3 className="text-sm font-semibold text-slate-900">
                Current booking screen users
              </h3>
              <p className="mt-1 text-xs text-slate-500">
                Passengers on booking and searching surfaces without an active
                booking code.
              </p>
              <div className="mt-3 overflow-auto">
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {scopedPresenceRows
                      .filter((row) => !row.last_booking_code)
                      .filter((row) => {
                        const screen = String(
                          row.screen_name || "",
                        ).toLowerCase();
                      ...
                      <tr>
                        <td colSpan={4} className="py-4 text-slate-400">
                          No current booking-screen users in this scope.
                        </td>
                      </tr>
                    ) : (
                      scopedPresenceRows
                        ...
                        .map((row) => (
                          ...
                          >
                            <td className="py-2 pr-4 font-medium text-slate-900">
                              {row.passenger_name || "Unknown Passenger"}
                            </td>
                            <td className="py-2 pr-4 text-slate-700">
                              {row.town || "Unknown"}
                            </td>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
            <h3 className="text-sm font-semibold text-slate-900">
              Recent active passengers
            </h3>
            <p className="mt-1 text-xs text-slate-500">
              Most recent fresh presence rows in the selected town scope.
            </p>
            <div className="mt-3 overflow-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    <th className="py-2 pr-4 font-semibold">Passenger</th>
                    <th className="py-2 pr-4 font-semibold">Town</th>
                    <th className="py-2 pr-4 font-semibold">App state</th>
                    <th className="py-2 pr-4 font-semibold">Screen</th>
                    <th className="py-2 pr-4 font-semibold">Booking code</th>
                ...
                <tbody>
                  {scopedPresenceRows.length === 0 ? (
                    ...
                    scopedPresenceRows.slice(0, 25).map((row, idx) => (
                      <tr
                        key={String(
                          row.passenger_id ||
                            `${row.last_seen_at || "row"}-${idx}`,
                        )}
                        className="border-b border-slate-100 last:border-0"
                      >
                        <td className="py-2 pr-4 font-medium text-slate-900">
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {scopedTrips.length === 0 ? (
                    ...
                    scopedTrips.map((row) => (
                      <tr
                        key={String(row.town || "Unknown (legacy data)")}
                        className="border-b border-slate-100 last:border-0"
                      >
                        <td className="py-3 pr-4 font-medium text-slate-900">
                          {row.town || "Unknown (legacy data)"}
                        </td>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold tracking-tight text-slate-900">
              Top drivers with TODA identification
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Completed trips, gross proposed fares, company share, TODA share,
              and quality signals.
            </p>
            <div className="mt-4 overflow-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    ...
                    <th className="py-3 pr-4 font-semibold">TODA</th>
                    <th className="py-3 pr-4 font-semibold">Trips</th>
                    <th className="py-3 pr-4 font-semibold">Gross fares</th>
                    <th className="py-3 pr-4 font-semibold">Company</th>
                    <th className="py-3 pr-4 font-semibold">TODA</th>
                    <th className="py-3 pr-4 font-semibold">Avg rating</th>
                  ...
                  {scopedDrivers.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="py-6 text-slate-400">
                        No driver analytics rows for the selected scope.
                      </td>
                    </tr>
                  ) : (
                    scopedDrivers.map((row) => (
                      <tr
                        key={String(
                          row.driver_id || row.driver_name || Math.random(),
                        )}
                        className="border-b border-slate-100 last:border-0"
                      >
                        <td className="py-3 pr-4 font-medium text-slate-900">
                        ...
                        <td className="py-3 pr-4 text-slate-700">
                          {row.municipality || "Unknown (legacy data)"}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {row.toda_name || "-"}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {formatCount(Number(row.completed_trips || 0))}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {formatPeso(
                            Number(row.gross_proposed_fare_earnings || 0),
                          )}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {formatPeso(
                            Number(
                              (row.total_company_share ??
                                row.total_platform_revenue) ||
                                0,
                            ),
                          )}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {formatPeso(Number(row.total_toda_share || 0))}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {Number(row.average_rating || 0).toFixed(2)}
                        </td>
                        <td className="py-3 text-slate-700">
                          {formatCount(Number(row.ratings_count || 0))}
                        </td>
                      </tr>
                    ))
                  )}
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold tracking-tight text-slate-900">
              TODA breakdown by town
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Each TODA receives PHP 1 per completed ride when the driver is
              identified as a TODA member.
            </p>
            <div className="mt-4 space-y-4">
              {scopedTrips.map((row) => (
                <div
                  key={`toda-${row.town || "Unknown (legacy data)"}`}
                ...
                >
                  ...
                  <div className="mt-1 text-sm text-slate-500">
                    TODA rides:{" "}
                    {formatCount(Number(row.toda_completed_trips || 0))} - TODA
                    share: {formatPeso(Number(row.toda_share_total || 0))}
                  </div>
                  <div className="mt-3 overflow-auto">
                    <table className="min-w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-200 text-left text-slate-500">
                          <th className="py-2 pr-4 font-semibold">TODA</th>
                          <th className="py-2 pr-4 font-semibold">Trips</th>
                          <th className="py-2 font-semibold">Share</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Array.isArray(row.toda_breakdown) &&
                        row.toda_breakdown.length > 0 ? (
                          row.toda_breakdown.map((item, idx) => (
                            <tr
                              key={`${row.town || "Unknown (legacy data)"}-${item.toda_name || idx}`}
                              className="border-b border-slate-100 last:border-0"
                            >
                              <td className="py-2 pr-4 text-slate-700">
                                {item.toda_name || "-"}
                              ...
                              <td className="py-2 pr-4 text-slate-700">
                                {formatCount(Number(item.trips || 0))}
                              </td>
                              <td className="py-2 text-slate-700">
                                {formatPeso(Number(item.toda_share_total || 0))}
                              </td>
                            </tr>
                          ))
                        ) : (
                          <tr>
                            <td colSpan={3} className="py-3 text-slate-400">
                              No TODA-tagged rides in this town.
                            </td>
                          </tr>
                        )}
                      </tbody>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold tracking-tight text-slate-900">
              No-driver search demand
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Passengers who searched but were not able to get a driver. This
              reads from driver_search_failures only. It does not patch or infer
              missing rows.
            </p>
            <div className="mt-4 overflow-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    <th className="py-3 pr-4 font-semibold">Passenger</th>
                    <th className="py-3 pr-4 font-semibold">Town</th>
                    <th className="py-3 pr-4 font-semibold">Time (PHT)</th>
                    <th className="py-3 pr-4 font-semibold">Pickup</th>
                    <th className="py-3 pr-4 font-semibold">Dropoff</th>
                    <th className="py-3 pr-4 font-semibold">Requested</th>
                    <th className="py-3 font-semibold">Alternate</th>
                  </tr>
                </thead>
                <tbody>
                  {scopedFailures.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-6 text-slate-400">
                        No no-driver search rows yet in this scope. This page
                        remains read-only and does not change booking logging
                        behavior.
                      </td>
                    </tr>
                  ) : (
                    scopedFailures.map((row) => (
                      <tr
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-1">
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Low-rating watchlist for management review and partner-safe
              monitoring.
            </p>
            <div className="mt-4 overflow-auto">
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-left text-slate-500">
                    ...
                    <th className="py-3 pr-4 font-semibold">Town</th>
                    <th className="py-3 pr-4 font-semibold">Avg rating</th>
                    <th className="py-3 pr-4 font-semibold">Low ratings</th>
                    <th className="py-3 pr-4 font-semibold">Completed</th>
                    <th className="py-3 pr-4 font-semibold">Latest feedback</th>
                    <th className="py-3 font-semibold">
                      Latest rating at (PHT)
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {scopedWatch.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-6 text-slate-400">
                        No watchlist rows for the selected scope.
                      </td>
                    </tr>
                  ) : (
                    scopedWatch.map((row) => (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4 text-slate-700">
                          {formatCount(Number(row.low_ratings_count || 0))}
                        </td>
                        <td className="py-3 pr-4 text-slate-700">
                          {formatCount(Number(row.completed_trips || 0))}
                        </td>
                        ...
                        <td className="py-3 text-slate-700">
                          {formatPHDateTime(row.latest_rating_at)}
                        </td>
                      </tr>
                    ))
                  )}
