...
export default function VendorMonthlyReportPage() {
  ...
  const presetThisMonth = `/admin/reports/lgu?tab=vendor&view=monthly&month=${encodeURIComponent(thisMonth)}`;
  const presetMonthly = `/admin/reports/lgu?tab=vendor&view=monthly`;

  return (
    <div style={{ padding: 16 }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, margin: 0 }}>Vendor Monthly Report</h1>
      <div style={{ marginTop: 6, fontSize: 13, opacity: 0.7 }}>
        Read-only report launcher. Uses LGU export view with monthly presets. No API calls here.
      </div>

      <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
        <div style={card}>
          <div style={{ fontSize: 16, fontWeight: 800 }}>Presets</div>
          <div style={{ marginTop: 8, display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link href={presetThisMonth} style={btn}>
              Open This Month ({thisMonth})
            </Link>
            <Link href={presetMonthly} style={btn}>
              Open Monthly (choose month)
            </Link>
          </div>
