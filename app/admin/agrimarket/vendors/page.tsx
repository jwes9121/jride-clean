...
export default function VendorsPage() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-7"><div className="flex flex-wrap items-center justify-between gap-4"><h1 className="text-2xl font-bold">AgriMarket store names</h1><button disabled={loading} onClick={() => void refresh()} className="rounded-xl border bg-white px-4 py-3">{loading ? "Loading..." : "Refresh"}</button></div><p className="my-4 text-sm text-slate-600">Store names are required before publishing products or receiving orders. Passengers see the name when opening the store profile. Private contact and pickup details remain restricted.</p>{error && <p role="alert" className="mb-4 rounded-xl bg-red-50 p-4 text-red-800">{error}</p>}<div className="grid gap-4 sm:grid-cols-2">{vendors.map(vendor => <article key={vendor.id} className="rounded-2xl border bg-white p-5"><h2 className="break-words text-lg font-bold">{vendor.vendor_name || "Vendor name not set"}</h2><p className="mt-2 text-sm">Contact: {vendor.contact_name}</p><p className="mt-1 text-sm text-slate-600">{vendor.town} - {vendor.status} - {vendor.accepting_orders ? "Accepting orders" : "Orders paused"}</p></article>)}</div>{!loading && !error && !vendors.length && <p>No vendors yet.</p>}</main>;
}
