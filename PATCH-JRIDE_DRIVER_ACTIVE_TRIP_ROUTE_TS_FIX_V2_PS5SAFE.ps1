# PATCH-JRIDE_DRIVER_ACTIVE_TRIP_ROUTE_TS_FIX_V2_PS5SAFE.ps1
# Fix TS build error + return richer trip fields + ignore stale assigned trips.
# Target: app/api/driver/active-trip/route.ts
...
$TargetRel = "app\api\driver\active-trip\route.ts"
$Target = Join-Path $RepoRoot $TargetRel

Ok "== JRIDE Patch: Driver active-trip route TS fix (V2 / PS5-safe) =="
Ok ("RepoRoot: {0}" -f $RepoRoot)
Ok ("Target : {0}" -f $Target)

if (-not (Test-Path -LiteralPath $Target)) {
  Fail "[FAIL] Target file not found:"
  Fail ("  {0}" -f $Target)
  exit 1
}
...
$bakPath = Join-Path $bakDir ("driver-active-trip.route.ts.bak." + $stamp)
...
Ok "[OK] Patched route.ts (UTF-8 no BOM)"
Ok "== DONE =="
