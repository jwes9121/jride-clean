...
<#
REPLACE JRIDE WEB: admin verification page with parse-safe JSX
PS5-safe, ASCII-only
...
What it does:
- Replaces the whole file with a clean, parse-safe page
- Keeps same working API endpoints:
  /api/admin/passenger-verifications/pending
  /api/admin/passenger-verifications/forward
  /api/admin/passenger-verifications/decide
- Removes problematic raw symbols that can trigger JSX parse issues
#>
...
Write-Host "== REPLACE JRIDE WEB: admin verification page parse-safe (V1 / PS5-safe) ==" -ForegroundColor Cyan

$root = (Resolve-Path -LiteralPath $ProjRoot).Path
Write-Host "Root: $root"

$target = Join-Path $root "app\admin\verification\page.tsx"
$bakDir = Join-Path $root "_patch_bak"

EnsureDir (Split-Path $target -Parent)
$bak = BackupFile $target $bakDir "ADMIN_VERIFICATION_PARSE_SAFE_V1"
...
function Thumb(props: { url: string | null | undefined; label: string }) {
  if (!props.url) {
    return <div className="text-xs opacity-60">{props.label}: (no preview)</div>;
  }

  return (
    <div className="mt-2">
      <div className="text-xs opacity-80">{props.label}:</div>
      <a href={props.url} target="_blank" rel="noreferrer" className="inline-block mt-1">
        <img
          src={props.url}
          alt={props.label}
...
export default function AdminVerificationPage() {
  ...
  async function forward(passenger_id: string, notes: string) {
    ...
    try {
      ...
      if (!r.ok || !j?.ok) {
        throw new Error(j?.error || ("Forward failed (HTTP " + r.status + ")"));
      }
  ...
  async function decide(passenger_id: string, decision: "approve" | "reject", admin_notes: string) {
    ...
    try {
      ...
      const j: any = await r.json().catch(() => ({}));
      if (!r.ok || !j?.ok) {
        throw new Error(j?.error || ("Decision failed (HTTP " + r.status + ")"));
      }
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="text-sm opacity-70 mt-1">
              Dispatcher queue: Submitted then forward to Pending Admin then approve or reject.
              Admin may also approve directly from Submitted.
            </div>
          </div>
          <button
            type="button"
