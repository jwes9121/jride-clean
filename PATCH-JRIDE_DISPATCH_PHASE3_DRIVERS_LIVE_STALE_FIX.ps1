# PATCH-JRIDE_DISPATCH_PHASE3_DRIVERS_LIVE_STALE_FIX.ps1
# Phase 3.1: drivers-live realtime timestamp + dedupe (schema-flex)
# - Prefer driver_locations (or equivalent) for location_updated_at
# - Deduplicate by driver_id keeping newest timestamp
# - Merge wallet fields from wallet/live view if available
...
if (!(Test-Path $target)) { Fail "Missing file: $target (run from repo root)" }
...
# New route.ts content (full replace)
$code = @'
...
Write-Host "  2) Hit /api/dispatch/drivers-live and confirm _src includes loc:* and location_updated_at is recent."
Write-Host "  3) Deploy to prod (Vercel) and re-check Dispatch UI driver cards."
