# C:\Users\jwes9\Desktop\jride-clean-fresh\PATCH-NEXT_WALLETS_PHASE5C_VENDOR_ID_AUTOFILL.ps1
$ErrorActionPreference = "Stop"

function Fail($m){ throw $m }

$repo = "C:\Users\jwes9\Desktop\jride-clean-fresh"
$target = Join-Path $repo "app\api\admin\livetrips\page-data\route.ts"
...
# New route.ts content (safe enrichment + attaches wallet balances onto each trip)
$new = @'
...
Write-Host "[OK] Patched:`n  $target" -ForegroundColor Green

# Quick sanity: confirm keywords exist
$txt = Get-Content $target -Raw
if ($txt -notmatch "VENDOR_ID_AUTOFILL") {
  Write-Host "[WARN] Sanity marker not found (still ok), but patch should be present." -ForegroundColor Yellow
}
Write-Host "[DONE] NEXT_WALLETS_PHASE5C_VENDOR_ID_AUTOFILL applied." -ForegroundColor Green
