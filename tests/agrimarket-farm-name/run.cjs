...
function uiHarness(opt={}) {
 ...
 const Page=load('app/agrimarket/producer/profile/page.tsx',{
  ...
  '@/lib/agrimarket/farmer-towns':{AGRIMARKET_ACTIVE_TOWNS:['Lagawe','Hingyon','Banaue','Lamut']},
...
(async()=>{
 await test('profile uses atomic V3 RPC, trusted farmer and selected active town',async()=>{const h=apiHarness();const res=await h.api.POST(h.req({confirm_vendor_name:true,producer_id:'another'}));assert.equal(res.status,200);assert.equal(res.body.profile.vendor_name_locked,true);assert.equal(h.calls[0].name,'agrimarket_farmer_save_profile_v3');assert.equal(h.calls[0].args.p_producer_id,'own-farm');assert.equal(h.calls[0].args.p_town,'Lamut');assert.equal(h.calls[0].args.p_confirm_vendor_name,true);});
