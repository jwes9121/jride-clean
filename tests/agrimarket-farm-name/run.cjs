...
function apiHarness(opt = {}) {
 ...
 const api = load('app/api/agrimarket/producer/profile/route.ts',{
  ...
  '../../_lib/admin-farmer-location':{reverseGeocodeFarmerPin:async()=>({launch_eligible:true,town:'Lamut',label:'Pickup gate'})}
 });
...
function uiHarness(opt={}) {
 ...
 }, {window:{dispatchEvent:()=>{}},sessionStorage:{setItem:(key,value)=>notices.push({key,value})},fetch:async(url,options)=>{requests.push({url,options});if(options.method==='POST'){if(opt.saveFailure)return {status:409,ok:false,json:async()=>({ok:false,message:'Save rejected'})};current={...current,profile_complete:true,vendor_name_locked:true};}return {status:200,ok:true,json:async()=>({ok:true,profile:current,message:'Saved; current approval unchanged.'})};}}).default;
...
(async()=>{
 await test('profile uses atomic V2 RPC and trusted farmer, with explicit confirmation',async()=>{const h=apiHarness();const res=await h.api.POST(h.req({confirm_vendor_name:true,producer_id:'another'}));assert.equal(res.status,200);assert.equal(res.body.profile.vendor_name_locked,true);assert.equal(h.calls[0].name,'agrimarket_farmer_save_profile_v2');assert.equal(h.calls[0].args.p_producer_id,'own-farm');assert.equal(h.calls[0].args.p_confirm_vendor_name,true);});
