...
Set-Content -LiteralPath $apiLogin -Value $loginSrc -Encoding UTF8
Ok "[OK] Wrote: $apiLogin"
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            ...
            <input className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2 outline-none"
              value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Juan Dela Cruz" required />
          </div>
          ...
          <div>
            ...
            <input className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2 outline-none"
              value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Barangay / Street" />
          </div>

          <div>
            <label className="text-sm opacity-80">Town (optional)</label>
            <input className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2 outline-none"
              value={town} onChange={(e) => setTown(e.target.value)} placeholder="Lagawe / Kiangan / Banaue..." />
          </div>

          <div>
            <label className="text-sm opacity-80">Password</label>
            <input className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2 outline-none"
              value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" type="password" required />
          </div>
          ...
          <div className="text-sm opacity-80 text-center">
            Already have an account? <a className="text-blue-300 underline" href="/passenger-login">Login</a>
          </div>
        </form>
      </div>
    </main>
...
export default function PassengerLoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMsg("✅ Login OK. (Pilot) Now open the booking page/dashboard next.");
    } catch (err: any) {
    ...
    <main className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-black/30 p-6 backdrop-blur">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="text-sm opacity-80 text-center">
            Don’t have an account yet? <a className="text-blue-300 underline" href="/passenger-signup">Register here</a>
          </div>
        </form>
      </div>
    </main>
...
Set-Content -LiteralPath $pgLogin -Value $passengerLoginSrc -Encoding UTF8
Ok "[OK] Wrote: $pgLogin"

Ok "`n[DONE] Passenger signup+login (phone+password) wired to Supabase Auth (metadata)."
Warn "Ensure Vercel env has SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_SERVICE_ROLE) + SUPABASE_URL. Login route uses NEXT_PUBLIC_SUPABASE_ANON_KEY."

Info "`nNEXT STEPS:"
Info "1) npm.cmd run build"
Info "2) git add -A"
Info "3) git commit -m `"JRIDE_PHASE5 public passenger phone signup+login (Supabase Auth)`""
Info "4) git tag JRIDE_PHASE5_PUBLIC_SIGNUP_$ts"
Info "5) git push"
Info "6) git push --tags"
