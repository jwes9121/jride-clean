# PATCH-JRIDE_VERIFICATION_STEP4_SERVER_ROLE_LOCK_V1.ps1
# STEP 4: Server-side lock for verification routes
# - pending: requires auth + role (admin OR dispatcher)
# - decide: requires auth + role (admin ONLY)
# - secure-by-default (because signed URLs are sensitive)
# - full-file replace with backups
...
Write-Host ""
Write-Host "[NEXT] Set env vars on Vercel then run: npm run build"
