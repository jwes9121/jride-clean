...
type Props = { isOpen: boolean; onClose: () => void; onSave: (coords: { lat: number; lng: number } | null) => void; };
      ...
      <div className="absolute left-1/2 top-1/2 w-[900px] max-w-[95vw] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white shadow-xl">
        ...
        <div className="p-4">
          <div className="w-full h-[480px] rounded-lg overflow-hidden border">
            {isOpen && <MapboxMap center={[121.1157, 16.8042]} zoom={13} />}
          </div>
        </div>
        <div className="flex items-center justify-end gap-3 px-4 py-3 border-t">
          ...
          <button onClick={() => onSave(null)} className="px-3 py-1.5 rounded bg-black text-white">Save pickup</button>
        </div>
      </div>
    </div>
  );
