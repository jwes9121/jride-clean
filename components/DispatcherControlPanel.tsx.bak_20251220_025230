    ...
    return () => {
      ...
      try {
        if (ch) {
          ...
          // @ts-ignore Ã¢â‚¬â€œ not all clients expose it on the same object
    ...
    <div className="p-4 border rounded-xl bg-white space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600">
          {loading ? "LoadingÃ¢â‚¬Â¦" : `${drivers.length} drivers (${activeCount} active)`}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {drivers.slice(0, 8).map((d) => (
          <div key={d.driver_id} className="border rounded-lg p-3">
            ...
            <div className="text-xs text-gray-500">{d.phone || "Ã¢â‚¬â€"}</div>
