...
export default function ReviewedRouteMap({snapshot,token,onReady}: {
  ...
  useEffect(()=>{
    ...
    let cleanupMap: mapboxgl.Map | undefined;
    let alive=true;
    const markers: mapboxgl.Marker[]=[];
    try {
      const points=snapshot.points;
      const map: mapboxgl.Map = new mapboxgl.Map({container:host.current,accessToken:token,style:"mapbox://styles/mapbox/streets-v12",center:[points[0].lng,points[0].lat],zoom:8});
      cleanupMap=map;
    ...
    return ()=>{ alive=false; markers.forEach(m=>m.remove()); cleanupMap?.remove(); };
