# PATCH-JRIDE_PHASE11B_RIDE_UNVERIFIED_UX.ps1
# - UI-only: clearer unverified state + "Request Verification" UX (no backend)
# - Removes any debug bypass remnants
# - Rewrites app/ride/page.tsx (single-file patch)
...
# Write file (ASCII)
[System.IO.File]::WriteAllText($target, $new, [System.Text.Encoding]::ASCII)

# Sanity checks: ensure no debug bypass remnants
$final = Get-Content $target -Raw
if ($final -match "debugBypass") { Fail "Sanity check failed: debugBypass still present." }
if ($final -match "\?debug=1") { Fail "Sanity check failed: ?debug=1 still present." }
if ($final -match "debug:\s*") { Fail "Sanity check failed: debug field still present in request body." }
