# PATCH-JRIDE_PHASE12C_FARE_ACCEPT_REJECT_ENDPOINTS_V1.ps1
# ASCII-only. UTF8 NO BOM.
# Creates two passenger endpoints for fare accept/reject.
# Uses bookings columns: proposed_fare, passenger_fare_response, verified_fare.
...
$acceptPath = Join-Path $root "app\api\public\passenger\fare\accept\route.ts"
$rejectPath = Join-Path $root "app\api\public\passenger\fare\reject\route.ts"
...
export async function POST(req: Request) {
  try {
    ...
    const { data: upd, error: uErr } = await supabase
      .from("bookings")
      .update({ passenger_fare_response: "rejected" })
      .eq("id", booking_id)
...
WriteUtf8NoBom $acceptPath $acceptCode
WriteUtf8NoBom $rejectPath $rejectCode

Write-Host "[OK] Wrote: $acceptPath"
Write-Host "[OK] Wrote: $rejectPath"
Write-Host ""
Write-Host "NEXT:"
